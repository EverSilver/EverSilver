...
| **Repo root `src/`**    | Rust library crate **`eversilver`** (lib name) with **`eversilver-core`** CLI binary entrypoint (`src/main.rs`) — `src/core/` (transport: HTTP/JSON-RPC, CLI, dispatch, auth, event bus), `src/eversilver/*` domains. The QuickJS skills runtime has been removed; `src/eversilver/skills/` is metadata-only now. |
| **Skills registry**     | **[`eversilver/eversilver-skills`](https://github.com/eversilver/eversilver-skills)** on GitHub — canonical skill packages and TS build; not vendored in this tree.                                       |
| **`Cargo.toml`** (root) | Core crate; `cargo build --bin eversilver-core` produces the CLI binary. Helper binaries: `slack-backfill`, `gmail-backfill-3d` in `src/bin/`.                                                              |
...
**Skills registry:** Skill sources and the bundler live in **[github.com/eversilver/eversilver-skills](https://github.com/eversilver/eversilver-skills)**. The desktop app's skills catalog defaults to that GitHub slug; override with `VITE_SKILLS_GITHUB_REPO` (see [`app/src/utils/config.ts`](app/src/utils/config.ts)). Note: since the QuickJS runtime was removed, the desktop app no longer *executes* skill packages — it only discovers, installs metadata, and renders catalog entries. Skill execution surfaces are being rebuilt; check the current domain modules before assuming a skill can run end-to-end.
...
- **Core runs in-process** as a tokio task inside the Tauri host (sidecar removed in PR #1061). The host owns its lifetime via `core_process::CoreProcessHandle` in `app/src-tauri/src/core_process.rs`. Frontend RPC still goes over HTTP to `http://127.0.0.1:<port>/rpc` authenticated with a per-launch hex bearer in `EVERSILVER_CORE_TOKEN`; the Tauri command `core_rpc_token` exposes it to the renderer. Set `EVERSILVER_CORE_REUSE_EXISTING=1` to attach to an externally-started `eversilver-core` process for debugging.

**Where logic lives**

- **Rust (`eversilver` / repo root `src/`)**: **Business logic and execution**—domains, RPC, persistence, CLI behavior. Authoritative.
...
# Skills — author / build in the registry repo (eversilver/eversilver-skills).
...
This checklist is required for remote agents because Eversilver has several merge gates that are easy to miss in partial environments: Prettier, Rust formatting, TypeScript typecheck, focused Vitest coverage, controller dispatch parity, and Tauri vendored dependency availability. If a command cannot run in the remote environment, the PR body must report the exact blocked command and error instead of claiming validation passed.
...
**Rust config** uses a TOML-based `Config` struct (`src/eversilver/config/schema/types.rs`) with env var overrides applied in `src/eversilver/config/schema/load.rs`. Env vars override config file values at runtime (e.g. `EVERSILVER_API_URL` overrides `config.api_url`).
...
By default, `app/scripts/e2e-run-spec.sh` creates and cleans a temp `EVERSILVER_WORKSPACE`
automatically when the variable is not provided.

If you need a fixed workspace for debugging, provide one explicitly:

```bash
export EVERSILVER_WORKSPACE="$(mktemp -d)"
pnpm test:e2e:build
bash app/scripts/e2e-run-spec.sh test/e2e/specs/smoke.spec.ts smoke
rm -rf "$EVERSILVER_WORKSPACE"
```

- `EVERSILVER_WORKSPACE` redirects core config + workspace storage away from `~/.eversilver`.
...
Bundled prompts live under **`src/eversilver/agent/prompts/`** at the **repository root** (also bundled via `app/src-tauri/tauri.conf.json` `resources`). Loaders under `app/src/lib/ai/` use `?raw` imports, optional remote fetch, and in Tauri **`ai_get_config` / `ai_refresh_config`** for packaged content.
...
**Core lifecycle**: `core_process::CoreProcessHandle` spawns the in-process JSON-RPC server and authenticates inbound RPC with a hex bearer (`EVERSILVER_CORE_TOKEN`). Stale-listener policy (#1130): on conflict the handle probes `GET /`, decides if the listener is an Eversilver core, then `kill_pid_term` → `kill_pid_force` with PID revalidation guarding against PID reuse. `restart_core_process` / `start_core_process` Tauri commands let the frontend cycle it for updates.

Registered IPC commands (see [`gitbooks/developing/architecture/tauri-shell.md`](gitbooks/developing/architecture/tauri-shell.md)) include `greet`, `write_ai_config_file`, `ai_get_config`, `ai_refresh_config`, `core_rpc_relay`, `core_rpc_token`, `start_core_process`, `restart_core_process`, window commands, and `eversilver_*` daemon helpers.
...
- **`src/eversilver/` module layout**: **New** functionality must live in a **dedicated subdirectory** (e.g. `eversilver/my_domain/mod.rs` plus related files, or a new subfolder under an existing domain). Do **not** add new standalone `*.rs` files directly at `src/eversilver/` root (`dev_paths.rs` and `util.rs` are grandfathered).
...
- **Domain schema files**: For each domain, define controller schema metadata in a dedicated module inside the domain folder (example: `src/eversilver/cron/schemas.rs`) and export from the domain `mod.rs`.
...
- **Layering**: Implementation in `eversilver::<domain>/`, controllers in `eversilver::<domain>/rpc.rs`, routes/dispatch in `src/core/`.

The previous QuickJS / `rquickjs` skills runtime has been removed. `src/eversilver/skills/` now contains metadata helpers only (`ops_create`, `ops_discover`, `ops_install`, `ops_parse`, `inject`, `schemas`, `types`) — its `mod.rs` and `types.rs` carry the marker comment "Legacy … retained after QuickJS runtime removal." Do not assume the runtime can execute a `.skill` package end-to-end; check what `ops_install` and the current agent tool path actually do before planning a feature that needs it.

### Controller migration checklist

- `src/eversilver/<domain>/mod.rs`: keep export-focused, add `mod schemas;` and re-export:
...
- `src/eversilver/<domain>/schemas.rs` must define:
...
3. Create a `bus.rs` file **inside your domain module** (e.g. `src/eversilver/billing/bus.rs`) for subscriber implementations — each domain owns its handlers.
...
1. Define the **request and response types** in the domain (e.g. `src/eversilver/billing/bus.rs`). Use owned fields, `Arc`s, and channels — not borrows. Types only need `Send + 'static`, not `Serialize`.
...
In the parent **Eversilver** desktop app, **Tauri / Rust is a delivery vehicle**: windowing, process lifecycle, IPC to the core sidecar, and other host concerns. **Keep as much UI behavior and product logic as practical in TypeScript/React** (`app/`). Avoid growing Rust in the shell for flows that belong in the web layer unless there is a hard platform or security reason.

## Git workflow

- **GitHub issues on upstream** — File and track issues on **[eversilver/eversilver](https://github.com/eversilver/eversilver/)** ([Issues](https://github.com/eversilver/eversilver/issues)), not only a fork’s tracker, unless the workflow explicitly says otherwise.
...
- **Open pull requests on upstream** — Always create PRs against **[eversilver/eversilver](https://github.com/eversilver/eversilver)** ([pull requests](https://github.com/eversilver/eversilver/pulls)), not only a fork’s default remote, unless the workflow explicitly says otherwise.
...
1. **Specify against the current codebase** — Ground the design in **existing** domains, controller/registry patterns, and JSON-RPC naming (`eversilver.<namespace>_<function>`). Reuse or extend documented flows in [`gitbooks/developing/architecture.md`](gitbooks/developing/architecture.md) and sibling guides; avoid parallel architectures.
2. **Implement in Rust** — Add domain logic under `src/eversilver/<domain>/`, wire **schemas + registered handlers** into the shared registry, and land **unit tests** in the crate (`cargo test -p eversilver`, focused modules) until the feature is correct in isolation.
...
**Capability catalog** — When a change adds, removes, renames, relocates, or materially changes a user-facing feature, update **`src/eversilver/about_app/`** in the same work so the runtime capability catalog remains the source of truth for what the app can do.
...
- **`src/eversilver/`**: New features go in a **folder/module**, not new root-level `src/eversilver/*.rs` files (see Rust core section).
...
- **Core is in-process**: `core_rpc` reaches `http://127.0.0.1:<port>/rpc` (default port `7788`) authenticated with `EVERSILVER_CORE_TOKEN`. `scripts/stage-core-sidecar.mjs` no longer exists; `pnpm core:stage` is a no-op echo (sidecar removed in PR #1061). For standalone debugging: `./target/debug/eversilver-core serve` writes its token to `{workspace}/core.token` (default `~/.eversilver-staging/core.token` under `EVERSILVER_APP_ENV=staging`); public endpoints `GET /health`, `GET /schema`, `GET /events` need no auth.

---

_Last aligned with monorepo layout (`app/` + root `src/`), in-process core (no sidecar), QuickJS removed, skills catalog on GitHub (`eversilver/eversilver-skills`), and Tauri shell IPC as of `eversilver-app` v0.53.45 / repo `main`._
...
| **Core JSON-RPC server** | `./target/debug/eversilver-core serve` | 7788 | Rust core, writes bearer token to `~/.eversilver-staging/core.token` |

The app connects to a **remote staging backend** at `https://staging-api.eversilver.local` — there is no local backend to run.

### Running the core server standalone

The core generates a bearer token at startup written to `{workspace_dir}/core.token` (default `~/.eversilver-staging/core.token` when `EVERSILVER_APP_ENV=staging`). Read that file for authenticated RPC calls:

```bash
TOKEN=$(cat ~/.eversilver-staging/core.token)
...
# [eversilver] recent context, 2026-04-22 9:52am PDT
...
### Apr 22, 2026
2848 9:07a ✅ eversilver: All Three Review Branches Pushed to Fork Successfully
2849 " 🔵 eversilver review-daemon-lifecycle: Two Post-Push Issues — Unstaged Prettier Changes + Missing tauri-cef Vendor
2851 9:08a ✅ eversilver daemon lifecycle: Prettier Format Committed as Follow-Up
2855 9:09a ✅ eversilver: All Three Review Branches Fully Pushed — PRs Ready to Open
2857 9:10a 🔵 eversilver: GitHub Connector Cannot Create PRs to eversilver/eversilver — 403 Forbidden
2858 9:11a 🔵 eversilver webhooks-ingress: Session Stalled — Instruction Not Processed After 10+ Minutes
2860 " 🔵 eversilver webhooks: WebhooksDebugPanel Architecture for E2E Smoke Spec
2861 9:13a 🔵 eversilver webhooks-ingress: Full Spec Surface Mapped — RPC Log Strings + UI Navigation Path
2866 9:15a 🟣 eversilver webhooks-ingress: webhooks-ingress-flow.spec.ts Written
2869 9:18a ⚖️ eversilver Memory Refactor Plan: Trait Shape, L1 Pointer, and Missing Pieces
2871 " 🔵 eversilver Memory Architecture: Auto-Inject Pattern Has 3 Separate Implementations
2873 9:31a 🟣 eversilver: Draft PR Opened — Config Runtime Dir Refactor for Testability
2874 9:32a 🟣 eversilver: 3 More Draft PRs Opened — Threads Schema, Daemon Lifecycle, Webhooks E2E
2875 9:33a 🔵 eversilver Memory Namespace: 3 Auto-Inject Sites, Not 1
2876 " ⚖️ eversilver Memory Refactor: Breaking Trait Change + Flag-Off + ToolDiscovery Hybrid
2877 " ✅ Memory Namespace Refactor Plan Written to docs/plans/memory-namespace-refactor.md
2879 9:34a 🔵 eversilver Memory Trait: 15 Impls, Not 14; MemoryRecalled Has No Live Emit Site
2880 " 🔵 eversilver SQLite Schema: memory_docs Already Has namespace Column; Migration Scope Minimal
2881 " 🔵 eversilver Memory Trait Current Signatures: No Namespace Param on Any Method
2882 " 🔵 eversilver Eval Infra: Does Not Exist; Phase D Requires Bootstrap from Scratch
