...
Eversilver can run a local model on your machine for the workloads where keeping data on-device matters most: **memory embeddings, summary-tree building, and background reasoning loops**. It is **opt-in** and ships **off** by default.
...
| ------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **Memory embeddings**     | `all-minilm:latest`               | `src/eversilver/embeddings/ollama.rs` - used by the [Memory Tree](../obsidian-wiki/memory-tree.md) for vector search. |
| **Summary-tree building** | `gemma3:1b-it-qat` (configurable) | `src/eversilver/tree_summarizer/ops.rs` - source / topic / global summary builders for the Memory Tree.            |
| **Heartbeat loop**        | small chat model                  | `src/eversilver/heartbeat/` - periodic background reflection.                                                      |
| **Learning / reflection** | small chat model                  | `src/eversilver/learning/reflection.rs` - passes that consolidate what was learned.                                |
| **Subconscious**          | small chat model                  | `src/eversilver/subconscious/executor.rs` - background evaluation loop.                                            |
...
| **Vision**         | Same.                                                                                               |
| **STT**            | Backend-proxied transcription (`src/eversilver/voice/cloud_transcribe.rs`).                          |
...
Under the hood, Eversilver supports two local provider paths:
...
For Ollama, Eversilver talks to its OpenAI-compatible `/v1` endpoint where possible. That means:

* The `OpenAiCompatibleProvider` (`src/eversilver/providers/compatible.rs`) wraps Ollama exactly the way it wraps a remote OpenAI-style provider. No special-case code path.
...
* Models are pulled on demand by Ollama and cached in its own store. Eversilver doesn't ship the weights itself.

For LM Studio, set `local_ai.provider = "lm_studio"` and ensure LM Studio's local server is running. Eversilver defaults to `http://localhost:1234/v1`, probes `GET /v1/models`, and sends chat requests to `POST /v1/chat/completions`. You can override the endpoint with `local_ai.base_url`, `EVERSILVER_LM_STUDIO_BASE_URL`, or `LM_STUDIO_BASE_URL`.

## Opting in

Local AI is gated by two flags in the core config (`src/eversilver/config/schema/local_ai.rs`):
...
In the desktop app, **Settings → AI & Skills → Local AI** exposes presets, pick one ("embeddings only", "memory + reflection", "everything local") and the right combination of flags is set for you. Status (Ollama reachability, model availability, per-subsystem enablement) is surfaced live via `eversilver.local_ai_status`.
...
Eversilver handles the rest: lifecycle (`src/eversilver/local_ai/service/`), API clients (`ollama_api.rs`, `lm_studio_api.rs`), health checks, and graceful fallback to remote when the local provider disappears.

### LM Studio troubleshooting

* Confirm the LM Studio local server is enabled and reachable at `http://localhost:1234/v1`.
* Load the selected model in LM Studio before calling Eversilver. Diagnostics report `load_lm_studio_model` when the configured `local_ai.chat_model_id` is not present in `/v1/models`.
* If LM Studio uses a different port, set `local_ai.base_url` or `EVERSILVER_LM_STUDIO_BASE_URL`.
* LM Studio model downloads are managed inside LM Studio. Eversilver will not pull LM Studio models from the local asset-download controls.
