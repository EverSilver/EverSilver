...
/// (for example `~/.eversilver/users/{user_id}` in production or
/// `~/.eversilver-staging/users/{user_id}` when `EVERSILVER_APP_ENV=staging`);
/// otherwise it falls back to that root directory itself.
pub fn get_data_dir() -> Result<PathBuf, String> {
    let root_dir = crate::eversilver::config::default_root_eversilver_dir()
        .map_err(|e| format!("Cannot determine app data directory: {e}"))?;
    std::fs::create_dir_all(&root_dir)
        .map_err(|e| format!("Failed to create data directory: {e}"))?;

    let data_dir = if let Some(user_id) = crate::eversilver::config::read_active_user_id(&root_dir) {
        let user_dir = crate::eversilver::config::user_eversilver_dir(&root_dir, &user_id);
...
/// (for example `~/.eversilver/encryption.key` or `~/.eversilver-staging/encryption.key`).
