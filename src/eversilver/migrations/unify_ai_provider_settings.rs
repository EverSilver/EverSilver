...
//! [`crate::eversilver::providers::factory`] — addressing all eight workloads
//! uniformly:
//!
//! ```text
//! reasoning_provider, agentic_provider, coding_provider,
...
fn seed_cloud_providers(config: &mut Config, stats: &mut MigrationStats) {
    ...
    let oh_endpoint = config
        ...
        .unwrap_or_else(|| CloudProviderType::Eversilver.default_endpoint().to_string());
    ...
    config.cloud_providers.push(CloudProviderCreds {
        id: generate_provider_id("eversilver"),
...
fn set_primary_cloud(config: &mut Config, stats: &mut MigrationStats) {
    ...
    let oh = config
        .cloud_providers
        .iter()
        .find(|e| e.slug == "eversilver" || e.legacy_type.as_deref() == Some("eversilver"));
    if let Some(entry) = oh {
        config.primary_cloud = Some(entry.id.clone());
        stats.primary_cloud_set = true;
        log::debug!(
            "[migrations][unify-ai] primary_cloud set to eversilver entry id={}",
            entry.id
        );
    }
}
...
/// at api.eversilver.ai; staging and dev URLs use the same host pattern.
///
/// Matches only on the host component to avoid false positives from custom
/// endpoints that happen to contain "eversilver" in a path or query string.
fn looks_like_eversilver(url: &str) -> bool {
    ...
    host_no_port == "api.eversilver.ai"
        || host_no_port.ends_with(".eversilver.ai")
        // Allow bare "eversilver" for local/dev names (e.g. Docker compose service names).
        || host_no_port == "eversilver"
}
