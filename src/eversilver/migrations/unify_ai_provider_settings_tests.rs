...
use super::*;
use crate::eversilver::config::schema::{LocalAiConfig, LocalAiUsage};
...
fn make_legacy_config_local_on() -> Config {
    ...
    c.memory_tree.llm_backend = crate::eversilver::config::schema::LlmBackend::Local;
    c
}
...
fn empty_config_seeds_eversilver_entry() {
    ...
    assert_eq!(c.cloud_providers[0].slug, "eversilver");
...
fn legacy_inference_url_becomes_custom_entry() {
    ...
    c.model_routes
        .push(crate::eversilver::config::schema::ModelRouteConfig {
            hint: "reasoning".into(),
            model: "gpt-4o".into(),
        });
...
fn eversilver_inference_url_does_not_seed_custom() {
    let mut c = Config::default();
    c.inference_url = Some("https://api.eversilver.ai/v1".into());
    let _ = run(&mut c).expect("migration must succeed");
    // Only the eversilver entry should be seeded — no Custom entry.
    assert_eq!(c.cloud_providers.len(), 1);
    assert_eq!(c.cloud_providers[0].slug, "eversilver");
}
...
fn chat_workload_providers_left_unset() {
    ...
    // and the factory defaults them to "eversilver" at runtime.
