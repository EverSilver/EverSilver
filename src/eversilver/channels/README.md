...
- `pub fn build_system_prompt` — re-exported from `crate::eversilver::context::channels_prompt`.
...
- `src/eversilver/agent/` — inbound messages spawn or resume agent runs through `runtime/dispatch.rs`.
- `src/eversilver/credentials/` — per-channel auth tokens, refresh flow.
- `src/eversilver/config/schema/channels.rs` — runtime channel configuration.
- `src/eversilver/threads/` — thread state for platforms with native threading (Slack `thread_ts`).
- `src/eversilver/notifications/` — surface inbound deliveries to the UI.
- `src/eversilver/encryption/` — at-rest secret protection.
- `src/core/event_bus/` — emits `DomainEvent::Channel(*)`; `channels/bus.rs` registers `ChannelInboundSubscriber`.

## Called by

- `src/eversilver/threads/ops.rs` — thread lifecycle uses channel send paths.
- `src/eversilver/memory/conversations/bus.rs` — persists incoming channel messages as conversation memories.
- `src/eversilver/cron/bus.rs` — scheduled triggers can post via channels.
- `src/eversilver/config/schema/channels.rs` — config layer references channel types for validation.
