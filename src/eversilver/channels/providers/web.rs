...
use crate::eversilver::agent::profiles::{AgentProfile, AgentProfileStore, DEFAULT_PROFILE_ID};
use crate::eversilver::agent::Agent;
...
use crate::eversilver::threads::turn_state::{TurnStateMirror, TurnStateStore};
...
struct WebChatTaskResult {
    ...
    citations: Vec<crate::eversilver::agent::memory_loader::MemoryCitation>,
}
    ...
    while let Some(c) = chars.next() {
        match c {
            '"' => {
                ...
                let sanitized = crate::eversilver::providers::sanitize_api_error(trimmed);
                return Some(crate::eversilver::util::truncate_with_ellipsis(
                    &sanitized,
                    MAX_DETAIL_CHARS,
                ));
            }
    ...
    let handle = tokio::spawn(async move {
        ...
        match result {
            ...
            Err(err) => {
                ...
                if crate::eversilver::agent::error::is_max_iterations_error(&detailed) {
                    log::info!(
                        target: "web_channel",
...
async fn run_chat_task(
    ...
    if was_built_fresh {
        match crate::eversilver::memory::conversations::get_messages(
    ...
    let result = match crate::eversilver::providers::thread_context::with_thread_id(
        thread_id.to_string(),
...
fn spawn_progress_bridge(
    mut rx: tokio::sync::mpsc::Receiver<crate::eversilver::agent::progress::AgentProgress>,
    client_id: String,
    thread_id: String,
    request_id: String,
...
fn load_reflection_chunks_for_thread(
    ...
    thread_id: &str,
) -> Option<Vec<crate::eversilver::subconscious::SourceChunk>> {
    let messages = crate::eversilver::memory::conversations::get_messages(
    ...
    let reflection_id = first
        ...
        .to_string();
    let reflection =
        crate::eversilver::subconscious::store::with_connection(workspace_dir, |conn| {
            crate::eversilver::subconscious::reflection_store::get_reflection(conn, &reflection_id)
        })
        .ok()
        .flatten()?;
