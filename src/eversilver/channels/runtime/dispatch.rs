...
use crate::eversilver::agent::bus::{AgentTurnRequest, AgentTurnResponse, AGENT_RUN_TURN_METHOD};
...
use crate::eversilver::channels::routes::{
    get_or_create_provider, get_route_selection, handle_runtime_command_if_needed,
};
use crate::eversilver::channels::traits;
use crate::eversilver::channels::{Channel, SendMessage};
use crate::eversilver::composio::fetch_connected_integrations;
use crate::eversilver::config::Config;
use crate::eversilver::providers::{self, ChatMessage};
use crate::eversilver::tools::{orchestrator_tools, Tool};
...
mod scoping_tests {
    ...
    use crate::eversilver::tools::traits::{PermissionLevel, Tool, ToolCategory, ToolResult};
...
pub(crate) async fn process_channel_message(
    ...
    if ctx.auto_save_memory {
        ...
        let _ = ctx
            ...
            .store(
                ...
                &autosave_key,
                &msg.content,
                crate::eversilver::memory::MemoryCategory::Conversation,
                None,
            )
            .await;
    }
    ...
    // `crate::eversilver::agent::bus::register_agent_handlers`); this keeps
    ...
    let (success, response_text) = match llm_result {
        ...
        Ok(Err(e)) => {
            ...
            // arm is unchanged.
            if crate::eversilver::agent::error::is_max_iterations_error(&e.to_string()) {
                log::info!(
                    target: "channels",
