...
use crate::eversilver::agent::harness::build_tool_instructions_filtered;
use crate::eversilver::agent::host_runtime;
use crate::eversilver::channels::context::{
...
use crate::eversilver::channels::dingtalk::DingTalkChannel;
use crate::eversilver::channels::discord::DiscordChannel;
use crate::eversilver::channels::email_channel::EmailChannel;
use crate::eversilver::channels::imessage::IMessageChannel;
use crate::eversilver::channels::irc;
use crate::eversilver::channels::irc::IrcChannel;
use crate::eversilver::channels::lark::LarkChannel;
use crate::eversilver::channels::linq::LinqChannel;
#[cfg(feature = "channel-matrix")]
use crate::eversilver::channels::matrix::MatrixChannel;
use crate::eversilver::channels::mattermost::MattermostChannel;
use crate::eversilver::channels::qq::QQChannel;
use crate::eversilver::channels::signal::SignalChannel;
use crate::eversilver::channels::slack::SlackChannel;
use crate::eversilver::channels::telegram::TelegramChannel;
use crate::eversilver::channels::traits;
use crate::eversilver::channels::whatsapp::WhatsAppChannel;
#[cfg(feature = "whatsapp-web")]
use crate::eversilver::channels::whatsapp_web::WhatsAppWebChannel;
use crate::eversilver::channels::Channel;
use crate::eversilver::config::Config;
use crate::eversilver::context::channels_prompt::build_system_prompt;
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    crate::eversilver::skills::bus::register_skill_cleanup_subscriber();
    ...
    // alive for the lifetime of the process (same pattern as TracingSubscriber).
    {
        ...
        EMAIL_SIG_HANDLE.get_or_init(|| {
            crate::eversilver::learning::extract::signature::register_email_signature_subscriber()
        });
    }
    ...
    // client is initialised later in `start_channels`).
    {
        ...
        REBUILD_TRIGGER_HANDLE.get_or_init(|| {
            if let Some(client) = crate::eversilver::memory::global::client_if_ready() {
                use crate::eversilver::learning::cache::FacetCache;
                use crate::eversilver::learning::scheduler::register_event_trigger;
                use crate::eversilver::learning::StabilityDetector;
                use std::sync::Arc;
                ...
                crate::eversilver::learning::scheduler::spawn_rebuild_loop(
                    Arc::clone(&detector),
                    crate::eversilver::learning::scheduler::DEFAULT_REBUILD_INTERVAL,
                    shutdown_rx,
                );
    ...
    // PROFILE.md blocks (style, identity, tooling, vetoes, goals).
    {
        ...
        PROFILE_MD_RENDERER_HANDLE.get_or_init(|| {
            if let Some(client) = crate::eversilver::memory::global::client_if_ready() {
                use crate::eversilver::learning::cache::FacetCache;
                use crate::eversilver::learning::ProfileMdRenderer;
                use std::sync::Arc;
    ...
    if let Err(err) = crate::eversilver::agent::harness::AgentDefinitionRegistry::init_global(
    ...
    let skills = crate::eversilver::skills::load_skills(&workspace);
    ...
    // subagent via category_filter = "skill".
    let non_skill_tools: Vec<&Box<dyn crate::eversilver::tools::Tool>> = tools_registry
        .iter()
        .filter(|t| t.category() != crate::eversilver::tools::traits::ToolCategory::Skill)
        .collect();
    let non_skill_refs: Vec<&dyn crate::eversilver::tools::Tool> =
    ...
    let _cron_delivery_handle = bus.subscribe(Arc::new(
        crate::eversilver::cron::bus::CronDeliverySubscriber::new(Arc::clone(&channels_by_name)),
    ));
    ...
    let _proactive_handle = bus.subscribe(Arc::new(
        crate::eversilver::channels::proactive::ProactiveMessageSubscriber::new(
    ...
    let _tree_summarizer_handle = bus.subscribe(Arc::new(
        crate::eversilver::tree_summarizer::bus::TreeSummarizerEventSubscriber::new(),
    ));
