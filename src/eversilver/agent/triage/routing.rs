...
use crate::eversilver::providers::{self, Provider, ProviderRuntimeOptions, INFERENCE_BACKEND_ID};

/// The concrete provider + metadata that [`crate::eversilver::agent::triage::evaluator::run_triage`]
/// should use for this particular triage turn.
pub struct ResolvedProvider {
    /// Ready-to-use provider, already constructed.
    pub provider: Arc<dyn Provider>,
    /// Provider name token — always `"eversilver"` (remote backend).
...
pub fn build_local_provider_with_config(config: &Config) -> Option<ResolvedProvider> {
    ...
    let (label, base) = if use_openai_compat {
        ...
        let ollama_base = crate::eversilver::local_ai::ollama_base_url();
