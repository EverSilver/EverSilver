...
use crate::eversilver::agent::harness::fork_context::{with_parent_context, ParentExecutionContext};
use crate::eversilver::agent::harness::subagent_runner::{self, SubagentRunOptions};
...
async fn dispatch_target_agent(agent_id: &str, prompt: &str) -> anyhow::Result<String> {
    ...
    let integrations = crate::eversilver::composio::fetch_connected_integrations(&config).await;
