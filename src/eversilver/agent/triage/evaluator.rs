...
//! `src/eversilver/agent/harness/tool_loop.rs` handles an empty registry
...
use crate::eversilver::config::MultimodalConfig;
use crate::eversilver::providers::reliable::{
    is_rate_limited, is_upstream_unhealthy, parse_retry_after_ms,
};
use crate::eversilver::providers::ChatMessage;
use crate::eversilver::scheduler_gate::LlmPermit;
...
pub async fn run_triage(envelope: &TriggerEnvelope) -> anyhow::Result<TriageOutcome> {
    ...
    let outcome = run_triage_with_arms_inner(cloud, local, envelope, || {
        crate::eversilver::scheduler_gate::wait_for_capacity()
    })
    .await;
...
///
/// Canonical sources:
/// - `src/eversilver/agent/bus.rs` — `Blocked` / `ReviewBlocked` arms of the
///   `enforce_prompt_input` decision (the path the triage evaluator hits via
///   `agent.run_turn`).
/// - `src/eversilver/agent/harness/session/runtime.rs` — same strings in the
///   tool-call loop, kept identical so this classifier covers both.
/// - `src/eversilver/local_ai/ops.rs` — user-facing variants with the
...
fn truncate_payload(payload: &serde_json::Value, max_bytes: usize) -> String {
    ...
    let end = crate::eversilver::util::floor_char_boundary(&pretty, max_bytes);
