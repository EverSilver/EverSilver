use super::*;
use crate::eversilver::agent::harness::definition::{ModelSpec, ToolScope};
...
fn make_def_named_tools(names: &[&str]) -> AgentDefinition {
    AgentDefinition {
        ...
        timeout_secs: None,
        sandbox_mode: crate::eversilver::agent::harness::definition::SandboxMode::None,
        background: false,
        subagents: vec![],
        delegate_name: None,
        source: crate::eversilver::agent::harness::definition::DefinitionSource::Builtin,
    }
}
...
use crate::eversilver::tools::{PermissionLevel, ToolResult};
...
use crate::eversilver::agent::harness::fork_context::with_parent_context;
use crate::eversilver::providers::{ChatRequest as PChatRequest, ChatResponse, Provider, ToolCall};
...
struct CapturedRequest {
    messages: Vec<crate::eversilver::providers::ChatMessage>,
    tool_count: usize,
}
...
fn make_parent(provider: Arc<dyn Provider>, tools: Vec<Box<dyn Tool>>) -> ParentExecutionContext {
    let tool_specs: Vec<crate::eversilver::tools::ToolSpec> =
    ...
    ParentExecutionContext {
        ...
        memory: noop_memory(),
        agent_config: crate::eversilver::config::AgentConfig::default(),
        skills: Arc::new(vec![]),
        memory_context: Arc::new(None),
        session_id: "test-session".into(),
        channel: "test".into(),
        connected_integrations: vec![],
        tool_call_format: crate::eversilver::context::prompt::ToolCallFormat::PFormat,
...
fn noop_memory() -> Arc<dyn crate::eversilver::memory::Memory> {
    struct NoopMemory;
    #[async_trait]
    impl crate::eversilver::memory::Memory for NoopMemory {
        async fn store(
            &self,
            _namespace: &str,
            _key: &str,
            _content: &str,
            _category: crate::eversilver::memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _query: &str,
            _limit: usize,
            _opts: crate::eversilver::memory::RecallOpts<'_>,
        ) -> anyhow::Result<Vec<crate::eversilver::memory::MemoryEntry>> {
            Ok(vec![])
        }
        async fn get(
            &self,
            _namespace: &str,
            _key: &str,
        ) -> anyhow::Result<Option<crate::eversilver::memory::MemoryEntry>> {
            Ok(None)
        }
        async fn list(
            &self,
            _namespace: Option<&str>,
            _category: Option<&crate::eversilver::memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<crate::eversilver::memory::MemoryEntry>> {
            Ok(vec![])
        }
        ...
        async fn namespace_summaries(
            &self,
        ) -> anyhow::Result<Vec<crate::eversilver::memory::NamespaceSummary>> {
            Ok(vec![])
        }
...
fn resolve_subagent_provider_hint_with_config_routes_via_factory() {
    ...
    config.agentic_provider = Some("eversilver".to_string());
...
fn direct_mode_user_with_stored_key_passes_signed_in_check() {
    ...
    let mut config = crate::eversilver::config::Config::default();
    ...
    config.composio.mode = crate::eversilver::config::schema::COMPOSIO_MODE_DIRECT.to_string();
