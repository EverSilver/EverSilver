...
//! [`crate::eversilver::agent::debug`] can mirror the live runner
...
use crate::eversilver::context::prompt::PromptContext;
...
/// [`crate::eversilver::agent::dispatcher::XmlToolDispatcher::prompt_instructions`]
...
/// agent builder ([`crate::eversilver::agent::harness::session::builder`])
...
///   ([`crate::eversilver::tools::orchestrator_tools::collect_orchestrator_tools`]
...
/// [`crate::eversilver::agent::debug`] shares the exact same
...
///
/// Exposed `pub(crate)` so the debug dump path in
/// [`crate::eversilver::agent::debug`] loads prompts through the
...
pub(crate) fn load_prompt_source(
    ...
    match source {
        ...
        PromptSource::File { path } => {
            ...
            // already shipped at `src/eversilver/agent/prompts/` and
