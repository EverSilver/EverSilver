...
use crate::eversilver::agent::harness::definition::{AgentDefinition, PromptSource};
use crate::eversilver::agent::harness::with_current_sandbox_mode;
use crate::eversilver::agent::progress::AgentProgress;
use crate::eversilver::context::prompt::{
    render_subagent_system_prompt, PromptContext, PromptTool, SubagentRenderOptions,
};
use crate::eversilver::memory::conversations::ConversationMessage;
use crate::eversilver::providers::{ChatMessage, ChatRequest, Provider, ToolCall};
use crate::eversilver::tools::{Tool, ToolCategory, ToolSpec};
...
pub(super) fn resolve_subagent_provider(
    spec: &crate::eversilver::agent::harness::definition::ModelSpec,
    agent_id: &str,
    config: Option<&crate::eversilver::config::Config>,
    parent_provider: std::sync::Arc<dyn Provider>,
    parent_model: String,
) -> (std::sync::Arc<dyn Provider>, String) {
    use crate::eversilver::agent::harness::definition::ModelSpec;
    match spec {
        ModelSpec::Hint(workload) => match config {
            Some(cfg) => match crate::eversilver::providers::create_chat_provider(workload, cfg) {
                Ok((p, m)) => {
                    log::info!(
...
/// [`crate::eversilver::composio::ComposioClient`] so the live
/// `composio.mode` toggle is honoured per execute — see
/// [`crate::eversilver::composio::ComposioActionTool`] and issue #1710.
struct LazyToolkitResolver {
    config: std::sync::Arc<crate::eversilver::config::Config>,
    actions: Vec<crate::eversilver::context::prompt::ConnectedIntegrationTool>,
}
...
impl LazyToolkitResolver {
    fn resolve(&self, name: &str) -> Option<Box<dyn Tool>> {
        ...
        Some(Box::new(
            crate::eversilver::composio::ComposioActionTool::new(
                self.config.clone(),
                action.name.clone(),
...
async fn run_typed_mode(
    ...
    let config_loaded = crate::eversilver::config::Config::load_or_init().await;
    ...
    let live_integrations: Vec<crate::eversilver::context::prompt::ConnectedIntegration> = {
        ...
        let probe_config = crate::eversilver::config::Config::load_or_init().await.ok();
        ...
        if !signed_in {
            ...
            match crate::eversilver::config::Config::load_or_init().await {
                Ok(config) => {
                    use crate::eversilver::composio::FetchConnectedIntegrationsStatus;
                    ...
                    // can't answer.
                    match crate::eversilver::composio::fetch_connected_integrations_status(&config)
                        .await
                    {
    ...
    if is_integrations_agent_with_toolkit {
        ...
        if let Some(tk) = toolkit_filter {
            ...
            let arc_config = match crate::eversilver::config::Config::load_or_init().await {
            ...
            // surface (#1710 Wave 2).
            use crate::eversilver::composio::client::{create_composio_client, ComposioClientKind};
                ...
                .find(|ci| ci.connected && ci.toolkit.eq_ignore_ascii_case(tk))
            {
                ...
                let fresh_actions = match &client_kind {
                    Some(ComposioClientKind::Backend(client)) => {
                        match crate::eversilver::composio::fetch_toolkit_actions(client, tk).await {
                ...
                let integration = crate::eversilver::context::prompt::ConnectedIntegration {
                ...
                let selected: Vec<&crate::eversilver::context::prompt::ConnectedIntegrationTool> =
    ...
    let narrowed_integrations: Vec<crate::eversilver::context::prompt::ConnectedIntegration> =
    ...
    // [`crate::eversilver::context::prompt`]. `Inline` / `File` sources
    ...
    let dispatcher_instructions = {
        ...
        use crate::eversilver::agent::pformat::PFormatRegistry;
        use crate::eversilver::context::prompt::ToolCallFormat;
    ...
    let prompt_ctx = PromptContext {
        ...
        learned: crate::eversilver::context::prompt::LearnedContextData::default(),
    ...
    let append_worker_message =
        |content: String, sender: String, extra_metadata: serde_json::Value| {
            if let Some(ref thread_id) = worker_thread_id {
                ...
                if let Err(err) = crate::eversilver::memory::conversations::append_message(
                    parent.workspace_dir.clone(),
                    thread_id,
                    message,
                ) {
    ...
    for iteration in 0..max_iterations {
        ...
        for call in &native_calls {
                ...
                .or_else(|| parent_tools.iter().find(|t| t.name() == call.name))
            {
                ...
                let timeout = crate::eversilver::tool_timeout::tool_execution_timeout_duration();
...
pub(crate) fn user_is_signed_in_to_composio(config: &crate::eversilver::config::Config) -> bool {
    crate::eversilver::composio::client::create_composio_client(config).is_ok()
}
