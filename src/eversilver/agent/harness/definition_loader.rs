...
//! with a fallback to `~/.eversilver/agents/*.toml` for user-global
...
/// `~/.eversilver/agents/` fallback. Returns an empty Vec when neither
/// directory exists.
...
/// injected by [`crate::eversilver::agent::agents::load_builtins`], so a
...
fn user_home_agents_dir() -> Option<PathBuf> {
    // Honour EVERSILVER_HOME first if set; otherwise ~/.eversilver.
    if let Ok(custom) = std::env::var("EVERSILVER_HOME") {
        return Some(PathBuf::from(custom).join("agents"));
    }
    match crate::eversilver::config::default_root_eversilver_dir() {
        Ok(dir) => Some(dir.join("agents")),
        Err(error) => {
            tracing::debug!(
                error = %error,
                "[agent-definition-loader] resolving root eversilver dir failed"
            );
            None
        }
    }
