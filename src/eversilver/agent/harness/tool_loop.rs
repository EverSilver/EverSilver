use crate::eversilver::agent::cost::TurnCost;
use crate::eversilver::agent::multimodal;
use crate::eversilver::agent::progress::AgentProgress;
use crate::eversilver::agent::stop_hooks::{current_stop_hooks, StopDecision, TurnState};
use crate::eversilver::approval::{ApprovalManager, ApprovalRequest, ApprovalResponse};
use crate::eversilver::providers::{
    ChatMessage, ChatRequest, Provider, ProviderCapabilityError, ProviderDelta,
};
use crate::eversilver::tools::traits::ToolScope;
...
use crate::eversilver::context::guard::{ContextCheckResult, ContextGuard};
...
pub(crate) async fn agent_turn(
    ...
    temperature: f64,
    silent: bool,
    multimodal_config: &crate::eversilver::config::MultimodalConfig,
...
pub(crate) async fn run_tool_call_loop(
    ...
    let tool_specs: Vec<crate::eversilver::tools::ToolSpec> = tools_registry
        .iter()
    ...
    for iteration in 0..max_iterations {
        ...
        match context_guard.check() {
            ...
            ContextCheckResult::CompactionNeeded => {
                tracing::warn!(
                    ...
                    crate::eversilver::context::guard::COMPACTION_TRIGGER_THRESHOLD * 100.0
                );
        ...
        let (response_text, parsed_text, tool_calls, assistant_history_content, native_tool_calls) =
            match chat_result {
                ...
                Err(e) => {
                    ...
                    let transient = crate::eversilver::providers::reliable::is_rate_limited(&e)
                        || crate::eversilver::providers::reliable::is_upstream_unhealthy(&e);
                    if transient {
                        tracing::warn!(
                            domain = "agent",
        ...
        for (call_idx, call) in tool_calls.iter().enumerate() {
            ...
            let result = if let Some(tool) = tool_opt {
                let tool_deadline =
                    crate::eversilver::tool_timeout::tool_execution_timeout_duration();
                let timeout_secs = crate::eversilver::tool_timeout::tool_execution_timeout_secs();
                    ...
                    Ok(Ok(r)) => {
                        ...
                        if success {
                            ...
                            let (compacted, tj_stats) =
                                crate::eversilver::tokenjuice::compact_tool_output(
                                    &call.name,
                                    Some(&call.arguments),
                                    &scrubbed,
                                    Some(0),
                            ...
                            let (compacted, _) = crate::eversilver::tokenjuice::compact_tool_output(
                                &call.name,
                                Some(&call.arguments),
                                &scrubbed,
                                Some(1),
