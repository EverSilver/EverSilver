...
//! [`crate::eversilver::agent::agents`] — each agent is a subfolder
...
/// [`crate::eversilver::agent::agents`] verify in CI that every entry in
/// [`crate::eversilver::agent::agents::BUILTINS`] still parses cleanly.
///
...
pub fn all() -> Vec<AgentDefinition> {
    #[allow(unused_mut)]
    let mut defs = crate::eversilver::agent::agents::load_builtins()
...
mod tests {
    ...
    fn all_definitions_present() {
        ...
        assert_eq!(
            defs.len(),
            crate::eversilver::agent::agents::BUILTINS.len() + 1
        );
    }
