...
use crate::eversilver::agent::dispatcher::{NativeToolDispatcher, XmlToolDispatcher};
use crate::eversilver::memory::Memory;
use crate::eversilver::providers::{ChatRequest, ConversationMessage, Provider};
...
struct MockProvider {
    responses: Mutex<Vec<crate::eversilver::providers::ChatResponse>>,
}
...
impl Provider for MockProvider {
    ...
    async fn chat(
        ...
        _model: &str,
        _temperature: f64,
    ) -> Result<crate::eversilver::providers::ChatResponse> {
        let mut guard = self.responses.lock();
        if guard.is_empty() {
            return Ok(crate::eversilver::providers::ChatResponse {
                text: Some("done".into()),
                tool_calls: vec![],
                usage: None,
            });
...
impl Tool for MockTool {
    ...
    ) -> Result<crate::eversilver::tools::ToolResult> {
        Ok(crate::eversilver::tools::ToolResult::success("tool-out"))
    }
}
...
fn build_minimal_agent_with_definition_name(definition_name: Option<&str>) -> Agent {
    ...
    let memory_cfg = crate::eversilver::config::MemoryConfig {
        backend: "none".into(),
        ..crate::eversilver::config::MemoryConfig::default()
    };
    let mem: Arc<dyn Memory> =
        Arc::from(crate::eversilver::memory::create_memory(&memory_cfg, &workspace_path).unwrap());
...
async fn turn_without_tools_returns_text() {
    ...
    let provider = Box::new(MockProvider {
        responses: Mutex::new(vec![crate::eversilver::providers::ChatResponse {
...
async fn turn_with_native_dispatcher_handles_tool_results_variant() {
    ...
    let provider = Box::new(MockProvider {
        responses: Mutex::new(vec![
            crate::eversilver::providers::ChatResponse {
                text: Some(String::new()),
                tool_calls: vec![crate::eversilver::providers::ToolCall {
                    id: "tc1".into(),
                    name: "echo".into(),
                    arguments: "{}".into(),
                }],
...
async fn turn_dispatches_spawn_subagent_through_full_path() {
    use crate::eversilver::agent::harness::AgentDefinitionRegistry;
    use crate::eversilver::tools::SpawnSubagentTool;
...
fn seed_resume_from_messages_is_noop_on_warm_agent() {
    ...
    agent.cached_transcript_messages = Some(vec![
        crate::eversilver::providers::ChatMessage::system("warm prefix"),
        crate::eversilver::providers::ChatMessage::user("hi"),
    ]);
    agent
