...
use crate::eversilver::agent::dispatcher::XmlToolDispatcher;
use crate::eversilver::agent::hooks::{PostTurnHook, TurnContext};
use crate::eversilver::agent::memory_loader::MemoryLoader;
use crate::eversilver::memory::Memory;
use crate::eversilver::providers::{ChatRequest, ChatResponse, Provider};
use crate::eversilver::tools::Tool;
use crate::eversilver::tools::ToolResult;
...
fn make_agent(visible_tool_names: Option<HashSet<String>>) -> Agent {
    ...
    let mut builder = Agent::builder()
        ...
        .config(crate::eversilver::config::AgentConfig {
            max_history_messages: 3,
            ..crate::eversilver::config::AgentConfig::default()
        });
...
fn make_agent_with_builder(
    ...
    config: crate::eversilver::config::AgentConfig,
    context_config: crate::eversilver::config::ContextConfig,
) -> Agent {
    let workspace = tempfile::TempDir::new().expect("temp workspace");
    let workspace_path = workspace.path().to_path_buf();
    std::mem::forget(workspace);
...
fn build_parent_context_and_sanitize_helpers_cover_snapshot_paths() {
    ...
    agent.skills = vec![crate::eversilver::skills::Skill {
        name: "demo".into(),
        ..Default::default()
    }];
...
async fn turn_runs_full_tool_cycle_with_context_and_hooks() {
    ...
    let mut agent = make_agent_with_builder(
        ...
        hooks,
        crate::eversilver::config::AgentConfig {
...
async fn execute_tool_call_applies_inline_result_budget() {
    ...
    let agent = make_agent_with_builder(
        ...
        vec![],
        crate::eversilver::config::AgentConfig::default(),
        crate::eversilver::config::ContextConfig {
