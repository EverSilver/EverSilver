...
use crate::eversilver::agent::dispatcher::{
    NativeToolDispatcher, PFormatToolDispatcher, ToolDispatcher, XmlToolDispatcher,
};
use crate::eversilver::agent::harness::definition::{
    AgentDefinitionRegistry, PromptSource, ToolScope,
};
use crate::eversilver::agent::host_runtime;
use crate::eversilver::agent::memory_loader::{DefaultMemoryLoader, MemoryLoader};
use crate::eversilver::config::{Config, ContextConfig};
use crate::eversilver::context::prompt::SystemPromptBuilder;
use crate::eversilver::context::{ContextManager, ProviderSummarizer};
use crate::eversilver::memory::{self, Memory};
use crate::eversilver::providers::{self, Provider};
use crate::eversilver::security::SecurityPolicy;
use crate::eversilver::tools::{self, Tool, ToolSpec};
use anyhow::Result;
use std::sync::Arc;
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: crate::eversilver::config::AgentConfig) -> Self {
    ...
    pub fn skills(mut self, skills: Vec<crate::eversilver::skills::Skill>) -> Self {
    ...
    pub fn post_turn_hooks(
        mut self,
        hooks: Vec<Arc<dyn crate::eversilver::agent::hooks::PostTurnHook>>,
    ) -> Self {
    ...
    /// [`crate::eversilver::agent::harness::payload_summarizer::PayloadSummarizer::maybe_summarize`]
    ...
    pub fn payload_summarizer(
        mut self,
        summarizer: Arc<
            dyn crate::eversilver::agent::harness::payload_summarizer::PayloadSummarizer,
        >,
    ) -> Self {
    ...
    pub fn build(self) -> Result<Agent> {
        ...
        let model_name = self
            .model_name
            .unwrap_or_else(|| crate::eversilver::config::DEFAULT_MODEL.into());
...
impl Agent {
    ...
    pub fn from_config_for_agent(config: &Config, agent_id: &str) -> Result<Self> {
        ...
        let target_def: Option<crate::eversilver::agent::harness::definition::AgentDefinition> =
    ...
    pub fn from_config_for_agent_with_reflection_chunks(
        config: &Config,
        agent_id: &str,
        reflection_chunks: Vec<crate::eversilver::subconscious::SourceChunk>,
    ) -> Result<Self> {
    ...
    pub fn from_config_for_agent_with_profile(
        config: &Config,
        agent_id: &str,
        reflection_chunks: Option<Vec<crate::eversilver::subconscious::SourceChunk>>,
    ...
    fn build_session_agent_inner(
        config: &Config,
        agent_id: &str,
        target_def: Option<&crate::eversilver::agent::harness::definition::AgentDefinition>,
        reflection_chunks: Option<Vec<crate::eversilver::subconscious::SourceChunk>>,
        profile_prompt_suffix: Option<String>,
    ) -> Result<Self> {
        ...
        if agent_id != "welcome" {
            tools.retain(|t| {
                !crate::eversilver::agent::harness::subagent_runner::is_welcome_only_tool(t.name())
            });
        }

        // Filter tools by user preference stored in app state.
        {
            use crate::eversilver::app_state::load_stored_app_state;
            match load_stored_app_state(config) {
                Ok(stored) => {
                    if let Some(ref tasks) = stored.onboarding_tasks {
                        if !tasks.enabled_tools.is_empty() {
                            crate::eversilver::tools::filter_tools_by_user_preference(
                                &mut tools,
                                &tasks.enabled_tools,
                            );
                        }
        ...
        let (provider, model_name): (Box<dyn Provider>, String) =
            crate::eversilver::providers::create_chat_provider("reasoning", config)?;
        ...
        if config.learning.enabled {
            ...
            prompt_builder = prompt_builder
                .insert_section_before(
                    "user_memory",
                    Box::new(crate::eversilver::context::prompt::UserReflectionsSection),
                )
                .add_section(Box::new(
                    crate::eversilver::learning::LearnedContextSection::new(memory.clone()),
                ))
                .add_section(Box::new(
                    crate::eversilver::learning::UserProfileSection::new(memory.clone()),
                ));
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            prompt_builder = prompt_builder.add_section(Box::new(
                crate::eversilver::agent::profiles::AgentProfilePromptSection::new(suffix),
            ));
        }

        // Build post-turn hooks when learning is enabled
        let mut post_turn_hooks: Vec<Arc<dyn crate::eversilver::agent::hooks::PostTurnHook>> =
            Vec::new();
        if config.learning.enabled {
            if config.learning.reflection_enabled {
                ...
                let reflection_provider: Option<Arc<dyn crate::eversilver::providers::Provider>> =
                    if config.learning.reflection_source
                        == crate::eversilver::config::ReflectionSource::Cloud
                    {
                ...
                post_turn_hooks.push(Arc::new(crate::eversilver::learning::ReflectionHook::new(
            ...
            if config.learning.user_profile_enabled {
                post_turn_hooks.push(Arc::new(crate::eversilver::learning::UserProfileHook::new(
                    config.learning.clone(),
                    memory.clone(),
                )));
            ...
            if config.learning.tool_tracking_enabled {
                post_turn_hooks.push(Arc::new(crate::eversilver::learning::ToolTrackerHook::new(
                    config.learning.clone(),
                    memory.clone(),
                )));
            ...
            if config.learning.tool_memory_capture_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::eversilver::memory::ToolMemoryCaptureHook::new(memory.clone(), true),
                ));
        ...
        let (delegation_tools, filter_from_scope): (
            ...
            target_def,
            crate::eversilver::agent::harness::definition::AgentDefinitionRegistry::global(),
        ) {
            (Some(def), Some(reg)) => {
        ...
        if config.learning.enabled {
            ...
            if has_retrieval {
                prompt_builder = prompt_builder
                    .add_section(Box::new(crate::eversilver::learning::MemoryAccessSection));
        ...
        let pformat_registry = crate::eversilver::agent::pformat::build_registry(&tools);
        ...
        {
            match crate::eversilver::agent::harness::definition::AgentDefinitionRegistry::global() {
                Some(reg) => match reg.get("summarizer") {
                    Some(summarizer_def) => {
                        ...
                        Some(std::sync::Arc::new(
                            crate::eversilver::agent::harness::payload_summarizer::SubagentPayloadSummarizer::new(
        ...
        let mut builder = Agent::builder()
            ...
            .skills(crate::eversilver::skills::load_skills(&config.workspace_dir))
...
fn prefetch_tool_memory_rules_blocking(
    ...
    tool_names: &[String],
) -> Vec<crate::eversilver::memory::ToolMemoryRule> {
    ...
    tokio::task::block_in_place(|| {
        handle.block_on(async move {
            let store = crate::eversilver::memory::ToolMemoryStore::new(memory);
...
mod dedup_tests {
    ...
    use crate::eversilver::tools::ToolSpec;
    use serde_json::json;
