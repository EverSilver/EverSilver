...
use crate::eversilver::agent::dispatcher::{ParsedToolCall, ToolExecutionResult};
use crate::eversilver::agent::harness;
use crate::eversilver::agent::hooks::{self, ToolCallRecord, TurnContext};
use crate::eversilver::agent::memory_loader::collect_recall_citations;
use crate::eversilver::agent::progress::AgentProgress;
use crate::eversilver::context::prompt::{LearnedContextData, PromptContext, PromptTool};
use crate::eversilver::context::{ReductionOutcome, ARCHIVIST_EXTRACTION_PROMPT};
use crate::eversilver::memory::MemoryCategory;
use crate::eversilver::providers::{ChatMessage, ChatRequest, ConversationMessage, ProviderDelta};
use crate::eversilver::tools::traits::ToolCallOptions;
use crate::eversilver::tools::Tool;
use crate::eversilver::util::truncate_with_ellipsis;
use anyhow::Result;
...
impl Agent {
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        if self.history.is_empty() {
            ...
            self.last_seen_integrations_hash =
                crate::eversilver::composio::connected_set_hash(&self.connected_integrations);
        } else {
            ...
            // [`crate::eversilver::composio::INTEGRATIONS_CACHE`], kept
            ...
            // [`crate::eversilver::composio::cached_active_integrations`]
            ...
            if let Ok(cfg) = crate::eversilver::config::Config::load_or_init().await {
                if let Some(cache_view) =
                    crate::eversilver::composio::cached_active_integrations(&cfg)
                {
                    let new_hash = crate::eversilver::composio::connected_set_hash(&cache_view);
        ...
        let context = if crate::eversilver::agent::tree_loader::should_prefetch(
            self.last_tree_prefetch_at,
            now,
            crate::eversilver::agent::tree_loader::REFRESH_INTERVAL,
        ) {
            match crate::eversilver::config::rpc::load_config_with_timeout().await {
                Ok(cfg) => {
                    match crate::eversilver::agent::tree_loader::TreeContextLoader::load(&cfg).await
                    {
                        Ok(tree_ctx) => {
        ...
        // docs on [`crate::eversilver::skills::inject`] for the matching
        // heuristic and size cap rationale.
        let enriched = {
            use crate::eversilver::skills::inject;
        ...
        let turn_body = async {
            ...
            Err(anyhow::Error::new(
                crate::eversilver::agent::error::AgentError::MaxIterationsExceeded {
    ...
    pub(super) async fn execute_tool_call(
        ...
        let (result, budget_outcome) =
            crate::eversilver::context::apply_tool_result_budget(raw_result, budget_bytes);
    ...
    pub(super) async fn fetch_learned_context(&self) -> LearnedContextData {
        ...
        let reflection_entries = self
            .memory
            .list(
                Some(crate::eversilver::learning::reflection::REFLECTIONS_NAMESPACE),
                Some(&MemoryCategory::Custom(
                    crate::eversilver::learning::reflection::REFLECTIONS_NAMESPACE.into(),
                )),
                None,
            )
            .await
    ...
    ///
    /// Delegates to the shared [`crate::eversilver::composio::fetch_connected_integrations`]
    ...
    /// [`crate::eversilver::composio::client::create_composio_client`]
    ...
    pub async fn fetch_connected_integrations(&mut self) {
        ...
        self.connected_integrations =
            crate::eversilver::composio::fetch_connected_integrations(&config).await;
    }
    ...
    pub fn refresh_delegation_tools(&mut self) -> bool {
        use crate::eversilver::agent::harness::definition::AgentDefinitionRegistry;
        use crate::eversilver::tools::orchestrator_tools::collect_orchestrator_tools;
        ...
        let synthed_specs: Vec<crate::eversilver::tools::ToolSpec> =
        ...
        let visible_specs: Vec<crate::eversilver::tools::ToolSpec> =
            if self.visible_tool_names.is_empty() {
    ...
    pub fn build_system_prompt(&self, learned: LearnedContextData) -> Result<String> {
        ...
        let ctx = PromptContext {
            ...
            connected_identities_md: crate::eversilver::agent::prompts::render_connected_identities(
            ),
            include_profile: !self.omit_profile,
            include_memory_md: !self.omit_memory_md,
            curated_snapshot: None,
            user_identity: crate::eversilver::app_state::peek_cached_current_user_identity(),
        };
    ...
    pub(super) fn persist_session_transcript(
        ...
        let meta = transcript::TranscriptMeta {
            ...
            cached_input_tokens,
            charged_amount_usd,
            thread_id: crate::eversilver::providers::thread_context::current_thread_id(),
        };
    ...
    /// heuristic [`crate::eversilver::learning::transcript_ingest`]
    ...
    pub(super) fn spawn_transcript_ingestion(&self) {
        ...
        tokio::spawn(async move {
            match crate::eversilver::learning::transcript_ingest::ingest_transcript_path(
                memory.as_ref(),
                &path,
            )
            .await
...
/// Wrapper around
/// [`crate::eversilver::tree_summarizer::store::collect_root_summaries_with_caps`]
/// that takes user-resolved per-namespace and total caps. The actual
/// limits are derived from the active
/// [`crate::eversilver::config::schema::agent::MemoryContextWindow`]
/// preset by [`crate::eversilver::config::schema::agent::AgentConfig::resolved_memory_limits`].
