...
use crate::eversilver::agent::dispatcher::ParsedToolCall;
use crate::eversilver::agent::error::AgentError;
use crate::eversilver::memory::Memory;
use crate::eversilver::prompt_injection::{
    enforce_prompt_input, PromptEnforcementAction, PromptEnforcementContext,
};
use crate::eversilver::providers::{self, ConversationMessage, Provider, ToolCall};
...
impl Agent {
    ...
    pub fn skills(&self) -> &[crate::eversilver::skills::Skill] {
        &self.skills
    }
    ...
    pub fn connected_integrations(
        &self,
    ) -> &[crate::eversilver::context::prompt::ConnectedIntegration] {
    ...
    pub fn set_connected_integrations(
        &mut self,
        integrations: Vec<crate::eversilver::context::prompt::ConnectedIntegration>,
    ) {
        self.connected_integrations = integrations;
    }

    /// The agent's runtime config snapshot.
    pub fn agent_config(&self) -> &crate::eversilver::config::AgentConfig {
        &self.config
    }
    ...
    pub fn set_on_progress(
        &mut self,
        tx: Option<tokio::sync::mpsc::Sender<crate::eversilver::agent::progress::AgentProgress>>,
    ) {
        self.on_progress = tx;
    }
    ...
    pub fn seed_resume_from_messages(
        ...
        let learned = crate::eversilver::agent::prompts::LearnedContextData::default();
        let system_prompt = self.build_system_prompt(learned)?;

        let mut cached: Vec<crate::eversilver::providers::ChatMessage> =
            Vec::with_capacity(prior.len() + 1);
        cached.push(crate::eversilver::providers::ChatMessage::system(
            system_prompt,
        ));
        for (role, content) in prior {
            let chat = match role.as_str() {
                "user" => crate::eversilver::providers::ChatMessage::user(content),
                "agent" | "assistant" => {
                    crate::eversilver::providers::ChatMessage::assistant(content)
                }
                ...
                _ => crate::eversilver::providers::ChatMessage::user(content),
            };
            cached.push(chat);
        }
    ...
    pub fn take_last_turn_citations(
        &mut self,
    ) -> Vec<crate::eversilver::agent::memory_loader::MemoryCitation> {
    ...
    pub(super) fn persisted_tool_calls_for_history(
        response: &crate::eversilver::providers::ChatResponse,
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        ...
        let cli = crate::eversilver::channels::CliChannel::new();

        let listen_handle = tokio::spawn(async move {
            let _ = crate::eversilver::channels::Channel::listen(&cli, tx).await;
        });
