...
use crate::eversilver::learning::transcript_ingest::CONVERSATION_MEMORY_NAMESPACE;
...
pub async fn collect_recall_citations(
    ...
    let entries = memory
        .recall(
            user_message,
            limit.max(1),
            crate::eversilver::memory::RecallOpts::default(),
        )
        .await?;
    ...
    let citations = entries
        ...
        .map(|entry| {
            let snippet = if entry.content.chars().count() > 280 {
                crate::eversilver::util::truncate_with_ellipsis(&entry.content, 280)
            } else {
                entry.content
            };
            MemoryCitation {
    ...
    async fn load_context(
        ...
        let prior_entries = memory
            .recall(
                ...
                crate::eversilver::memory::RecallOpts {
