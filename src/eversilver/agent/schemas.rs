...
use crate::eversilver::agent::profiles::{AgentProfile, AgentProfileStore};
...
fn handle_list_definitions(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        let registry = crate::eversilver::agent::harness::AgentDefinitionRegistry::global()
            .ok_or_else(|| "AgentDefinitionRegistry not initialised".to_string())?;
        let defs: Vec<&crate::eversilver::agent::harness::AgentDefinition> = registry.list();
...
fn handle_reload_definitions(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        ...
        let already_loaded =
            crate::eversilver::agent::harness::AgentDefinitionRegistry::global().is_some();
        Ok(serde_json::json!({
            "status": "noop",
    ...
    Box::pin(async move {
        ...
        let envelope = match p.source.as_str() {
            "composio" => {
                ...
                crate::eversilver::agent::triage::TriggerEnvelope::from_composio(
            ...
            "webhook" => {
                ...
                crate::eversilver::agent::triage::TriggerEnvelope::from_webhook(
            ...
            "cron" => {
                ...
                let output = p
                    ...
                    .unwrap_or(job_name);
                crate::eversilver::agent::triage::TriggerEnvelope::from_cron(
            ...
            "external" => {
                ...
                crate::eversilver::agent::triage::TriggerEnvelope::from_external(
        ...
        let outcome = crate::eversilver::agent::triage::run_triage(&envelope)
            .await
            .map_err(|e| format!("triage evaluation failed: {e}"))?;

        let dry_run = p.dry_run.unwrap_or(false);
        match outcome {
            crate::eversilver::agent::triage::TriageOutcome::Decision(run) => {
                if !dry_run {
                    crate::eversilver::agent::triage::apply_decision(run.clone(), &envelope)
                        .await
            ...
            crate::eversilver::agent::triage::TriageOutcome::Deferred {
                defer_until_ms,
                reason,
            } => {
...
fn handle_profile_upsert(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        if let Some(registry) = crate::eversilver::agent::harness::AgentDefinitionRegistry::global()
        {
...
mod tests {
    ...
    use crate::eversilver::agent::profiles::DEFAULT_PROFILE_ID;
    use crate::eversilver::config::TEST_ENV_LOCK as ENV_LOCK;
    use serde_json::json;
    ...
    async fn profile_upsert_rejects_unknown_registered_agent_id() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _ = crate::eversilver::agent::harness::AgentDefinitionRegistry::init_global_builtins();
