    ...
    async fn handle(&self, event: &DomainEvent) {
        match event {
            DomainEvent::SystemStartup { component } => {
                crate::eversilver::health::mark_component_ok(component);
            }
            DomainEvent::HealthChanged {
                ...
                if *healthy {
                    ...
                    crate::eversilver::health::mark_component_error(
                        component,
            ...
            DomainEvent::HealthRestarted { component } => {
                crate::eversilver::health::bump_component_restart(component);
            }
            DomainEvent::ChannelConnected { channel } => {
                crate::eversilver::health::mark_component_ok(&format!("channel:{channel}"));
            }
...
mod tests {
    ...
    async fn health_changed_false_records_error() {
        ...
        let snapshot = crate::eversilver::health::snapshot();
