...
impl McpHttpClient {
    ...
    async fn call_tool(&self, name: &str, arguments: Value) -> anyhow::Result<Value> {
        ...
        let builder =
            crate::eversilver::config::apply_runtime_proxy_to_builder(builder, "tool.gitbooks");
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "url": {
                    "type": "string",
                    "description": "The full URL of the Eversilver documentation page (e.g. https://eversilver.gitbook.local/eversilver/getting-started)."
                }
            },
            "required": ["url"]
        })
...
mod tests {
    ...
    fn redact_endpoint_keeps_only_origin() {
        assert_eq!(
            redact_endpoint("https://eversilver.gitbook.local/eversilver/~gitbook/mcp"),
    ...
    async fn live_search_smoke() {
        ...
        let t = GitbooksSearchTool::new(
            "https://eversilver.gitbook.local/eversilver/~gitbook/mcp".into(),
            30,
        );
        let result = t
            .execute(json!({"query": "what is eversilver"}))
            .await
            .unwrap();
        assert!(
            !result.is_error,
