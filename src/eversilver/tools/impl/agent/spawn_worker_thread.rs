...
use crate::eversilver::agent::harness::fork_context::current_parent;
use crate::eversilver::agent::harness::subagent_runner::{run_subagent, SubagentRunOptions};
use crate::eversilver::memory::conversations::{
    self, ConversationMessage, CreateConversationThread,
};
use crate::eversilver::tools::traits::{PermissionLevel, Tool, ToolResult};
...
impl Tool for SpawnWorkerThreadTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let current_thread_id = crate::eversilver::providers::thread_context::current_thread_id()
...
mod tests {
    ...
    use crate::eversilver::agent::harness::ParentExecutionContext;
    ...
    struct MockProvider;
    #[async_trait]
    impl crate::eversilver::providers::Provider for MockProvider {
        ...
        async fn chat(
            &self,
            _: crate::eversilver::providers::ChatRequest<'_>,
            _: &str,
            _: f64,
        ) -> anyhow::Result<crate::eversilver::providers::ChatResponse> {
            Ok(crate::eversilver::providers::ChatResponse {
                text: Some("done".into()),
                tool_calls: vec![],
                usage: None,
            })
    ...
    struct MockMemory;
    #[async_trait]
    impl crate::eversilver::memory::Memory for MockMemory {
        async fn store(
            &self,
            _: &str,
            _: &str,
            _: &str,
            _: crate::eversilver::memory::MemoryCategory,
            _: Option<&str>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        async fn recall(
            ...
            _: &str,
            _: usize,
            _: crate::eversilver::memory::RecallOpts<'_>,
        ...
        async fn list(
            &self,
            _: Option<&str>,
            _: Option<&crate::eversilver::memory::MemoryCategory>,
            _: Option<&str>,
        ) -> anyhow::Result<Vec<crate::eversilver::memory::MemoryEntry>> {
            Ok(vec![])
        }
    ...
    fn test_parent_ctx(workspace_dir: PathBuf) -> ParentExecutionContext {
        ParentExecutionContext {
            ...
            tool_call_format: crate::eversilver::context::prompt::ToolCallFormat::Native,
        }
    }
    ...
    async fn rejects_if_already_worker_thread() {
        ...
        crate::eversilver::providers::thread_context::with_thread_id(thread_id.to_string(), async {
