...
//!   heuristic (see `eversilver::webview_accounts`).
//!
...
pub(crate) async fn compute_state(
    ...
    let connected_channels =
        crate::eversilver::channels::controllers::connected_channel_slugs(config)
            .await
            .unwrap_or_else(|err| {
                tracing::warn!(
                    error = %err,
