...
use crate::eversilver::agent::host_runtime::{NativeRuntime, RuntimeAdapter};
use crate::eversilver::config::{Config, DelegateAgentConfig};
use crate::eversilver::memory::Memory;
use crate::eversilver::node_runtime::NodeBootstrap;
...
pub fn all_tools(
    ...
    browser_config: &crate::eversilver::config::BrowserConfig,
    http_config: &crate::eversilver::config::HttpRequestConfig,
    workspace_dir: &std::path::Path,
    agents: &HashMap<String, DelegateAgentConfig>,
    root_config: &crate::eversilver::config::Config,
) -> Vec<Box<dyn Tool>> {
...
pub fn all_tools_with_runtime(
    ...
    tools.push(Box::new(WebSearchTool::new(
        crate::eversilver::integrations::build_client(root_config),
    ...
    if root_config.seltz.enabled {
        tools.push(Box::new(
            crate::eversilver::integrations::SeltzSearchTool::new(
    ...
    if !agents.is_empty() {
        ...
        tools.push(Box::new(DelegateTool::new_with_options(
            delegate_agents,
            security.clone(),
            crate::eversilver::providers::ProviderRuntimeOptions {
    ...
    if let Some(client) = crate::eversilver::integrations::build_client(root_config) {
        tracing::debug!("[integrations] client built successfully");
        if root_config.integrations.apify.enabled {
            tools.push(Box::new(
                crate::eversilver::integrations::ApifyRunActorTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::ApifyGetRunStatusTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::ApifyGetRunResultsTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.google_places.enabled {
            tools.push(Box::new(
                crate::eversilver::integrations::GooglePlacesSearchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::GooglePlacesDetailsTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.parallel.enabled {
            tools.push(Box::new(
                crate::eversilver::integrations::ParallelSearchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::ParallelExtractTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::ParallelChatTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::ParallelResearchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::ParallelEnrichTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::ParallelDatasetTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.stock_prices.enabled {
            tools.push(Box::new(
                crate::eversilver::integrations::StockQuoteTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::StockExchangeRateTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::StockOptionsTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::StockCryptoSeriesTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::eversilver::integrations::StockCommodityTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.twilio.enabled {
            tools.push(Box::new(
                crate::eversilver::integrations::TwilioCallTool::new(Arc::clone(&client)),
            ));
        ...
        // `src/eversilver/composio/tools.rs` for per-tool details.
        let composio_tools = crate::eversilver::composio::all_composio_agent_tools(root_config);
    ...
    if crate::eversilver::tools::implementations::lsp_capability_enabled() {
        tools.push(Box::new(
            crate::eversilver::tools::implementations::LspTool::new(),
        ));
