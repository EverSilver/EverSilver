use super::*;
use crate::eversilver::config::{BrowserConfig, Config, MemoryConfig};
...
fn all_tools_includes_spawn_subagent() {
    ...
    let mem: Arc<dyn Memory> =
        Arc::from(crate::eversilver::memory::create_memory(&mem_cfg, tmp.path()).unwrap());
    ...
    let http = crate::eversilver::config::HttpRequestConfig::default();
