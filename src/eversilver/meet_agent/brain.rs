...
//!    to [`crate::eversilver::voice::cloud_transcribe`]. Returns the
...
//!    [`crate::eversilver::voice::reply_speech`] requesting
...
/// wake word fires so multi-caption utterances ("hey eversilver …
...
pub async fn run_caption_turn(request_id: &str) -> Result<bool, String> {
    // Wait briefly so a multi-fragment wake utterance ("hey eversilver
...
async fn stt(samples: &[i16]) -> Result<String, String> {
    use crate::eversilver::voice::cloud_transcribe::{transcribe_cloud, CloudTranscribeOptions};

    let config = crate::eversilver::config::ops::load_config_with_timeout().await?;
...
the speaker addresses you by name (\"Eversilver\", \"hey eversilver\"), asks a direct \
...
async fn tts(text: &str) -> Result<Vec<i16>, String> {
    use crate::eversilver::voice::reply_speech::{synthesize_reply, ReplySpeechOptions};
...
mod tests {
    use super::*;
    use crate::eversilver::meet_agent::session::registry;
