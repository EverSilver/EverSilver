...
impl MeetAgentSession {
    ...
    /// substring on `"hey eversilver"` (and `"hey open human"` to
    ...
    pub fn note_caption(&mut self, speaker: &str, text: &str, ts_ms: u64) -> bool {
        ...
        // phrase ("hey, eversilver"), capitalizes mid-sentence, and
        ...
        if let Some(idx) = wake_idx {
            let after = if normalized[idx..].starts_with("hey eversilver") {
                idx + "hey eversilver".len()
            } else {
...
mod tests {
    ...
    fn note_caption_does_not_double_fire_on_growing_caption() {
        let mut s = MeetAgentSession::new("p".into(), 16_000);
        let first = s.note_caption("Alice", "hey eversilver take notes", 1);
        assert!(first);
        let second = s.note_caption("Alice", "hey eversilver take notes about the launch", 2);
        assert!(!second, "second caption while wake_active must not refire");
        let prompt = s.take_pending_prompt().expect("prompt drained");
        // First wake stripped "hey eversilver"; the continuation
        // appended the WHOLE growing caption (still containing "hey
        // eversilver" because we don't re-strip), separated by a
