...
pub(crate) struct NativeChatRequest {
    ...
    /// set — see `crate::eversilver::providers::thread_context`).
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// (carrying token counts and `eversilver.billing.charged_amount_usd`
...
/// `crate::eversilver::providers::traits::StreamOptions`, which is the
...
pub(crate) struct ApiChatResponse {
    ...
    #[serde(default)]
    pub(crate) eversilver: Option<EversilverMeta>,
}
