//! Inference via the Eversilver backend OpenAI-compatible API (`{api_url}/openai/v1/...`) using the app session JWT.
//! Session material is loaded via [`crate::eversilver::credentials`] (see also [`crate::api::jwt`] for shared helpers).
...
pub const PROVIDER_LABEL: &str = "Eversilver";

/// Routes chat to `config.api_url` + `/openai` with `Authorization: Bearer` from the `app-session` profile.
pub struct EversilverBackendProvider {
    options: ProviderRuntimeOptions,
    api_url: Option<String>,
}

impl EversilverBackendProvider {
    ...
    fn state_dir(&self) -> PathBuf {
        self.options.eversilver_dir.clone().unwrap_or_else(|| {
    ...
    fn resolve_bearer(&self) -> anyhow::Result<String> {
        ...
        // 5,414 Sentry events on issue EVERSILVER-TAURI-1T.
        //
        ...
        if crate::eversilver::scheduler_gate::is_signed_out() {
            anyhow::bail!(
    ...
    fn inner(&self, token: &str) -> anyhow::Result<OpenAiCompatibleProvider> {
        // Hosted Eversilver API is chat-completions only; skip /v1/responses fallback so transport
        ...
        // never see this field (see `with_eversilver_thread_id`).
        ...
        .with_eversilver_thread_id())
    }
}

#[async_trait]
impl Provider for EversilverBackendProvider {
    ...
    fn stream_chat_with_system(
        ...
        stream::once(async move {
            Ok(StreamChunk::error(
                "streaming is not supported for Eversilver backend provider",
            ))
        })
        .boxed()
    }
