...
//!
//! Exposes `eversilver.providers_list_models` — fetches the `/models` endpoint
...
fn handle_list_models(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let api_key =
            crate::eversilver::providers::factory::lookup_key_for_slug(&entry.slug, &config)
                .unwrap_or_default();
        ...
        let client = crate::eversilver::config::build_runtime_proxy_client_with_timeouts(
        ...
        request = match entry.auth_style {
            ...
            AuthStyle::EversilverJwt | AuthStyle::None => request,
        };
        ...
        if !status.is_success() {
            ...
            let truncated = crate::eversilver::util::truncate_with_ellipsis(&body, 300);
            return Err(format!(
                "provider returned {}: {}",
                status.as_u16(),
                truncated
