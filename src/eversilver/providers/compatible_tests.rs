...
/// the Eversilver backend's `eversilver.billing` block also never lands,
...
async fn outbound_thread_id_is_gated_per_provider() {
    ...
    let eversilver =
        make_provider("Eversilver", "https://api.eversilver.test", None).with_eversilver_thread_id();
    ...
    with_thread_id("thread-xyz", async {
        ...
        assert_eq!(
            eversilver.outbound_thread_id().as_deref(),
            Some("thread-xyz"),
...
fn prompt_guided_tool_fallback_injects_system_instruction() {
    ...
    let tools = vec![crate::eversilver::tools::ToolSpec {
...
fn tool_specs_convert_to_openai_format() {
    let specs = vec![crate::eversilver::tools::ToolSpec {
