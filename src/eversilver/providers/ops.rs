...
pub const INFERENCE_BACKEND_ID: &str = "eversilver";
...
pub fn sanitize_api_error(input: &str) -> String {
    ...
    crate::eversilver::util::truncate_with_ellipsis(&scrubbed, MAX_API_ERROR_CHARS)
}
...
pub fn format_error_chain(err: &dyn std::error::Error) -> String {
    ...
    crate::eversilver::util::truncate_with_suffix(&scrubbed, TRANSPORT_ERROR_MAX_CHARS, "…")
}
...
pub fn create_backend_inference_provider(
    ...
    if let (Some(url), Some(key)) = (inference_url, api_key) {
        ...
        Ok(Box::new(
            crate::eversilver::providers::compatible::OpenAiCompatibleProvider::new(
                "custom_openai",
                url,
                Some(key),
                crate::eversilver::providers::compatible::AuthStyle::Bearer,
            ),
        ))
    } else {
...
pub fn create_resilient_provider(
    ...
    backend_url: Option<&str>,
    api_key: Option<&str>,
    reliability: &crate::eversilver::config::ReliabilityConfig,
) -> anyhow::Result<Box<dyn Provider>> {
...
pub fn create_routed_provider(
    ...
    api_key: Option<&str>,
    reliability: &crate::eversilver::config::ReliabilityConfig,
    model_routes: &[crate::eversilver::config::ModelRouteConfig],
    default_model: &str,
) -> anyhow::Result<Box<dyn Provider>> {
...
pub fn create_intelligent_routing_provider(
    ...
    let default_model = config
        .default_model
        .as_deref()
        .unwrap_or(crate::eversilver::config::DEFAULT_MODEL);
    ...
    let provider = crate::eversilver::routing::new_provider(remote, &config.local_ai, default_model);
    Ok(Box::new(provider))
}
...
pub fn list_providers() -> Vec<ProviderInfo> {
    vec![ProviderInfo {
        ...
        aliases: &["backend", "eversilver-backend"],
        local: false,
    }]
}
