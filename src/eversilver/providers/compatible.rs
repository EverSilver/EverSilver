...
use crate::eversilver::providers::traits::{
...
impl OpenAiCompatibleProvider {
    ...
    fn http_client(&self) -> Client {
        if let Some(ua) = self.user_agent.as_deref() {
            ...
            let builder = crate::eversilver::config::apply_runtime_proxy_to_builder(
                builder,
                "provider.compatible",
            );
    ...
    fn tool_specs_to_openai_format(
        tools: &[crate::eversilver::tools::ToolSpec],
    ...
    fn convert_tool_specs(
        tools: Option<&[crate::eversilver::tools::ToolSpec]>,
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions = crate::eversilver::providers::traits::build_tool_instructions_text(tools);
    ...
    async fn stream_native_chat(
        ...
        delta_tx: &tokio::sync::mpsc::Sender<crate::eversilver::providers::ProviderDelta>,
        dump_seq: u64,
    ) -> anyhow::Result<ProviderChatResponse> {
        ...
        while let Some(item) = bytes_stream.next().await {
            ...
            while let Some(sep_idx) = buffer.find("\n\n") {
                ...
                for line in event.lines() {
                    ...
                    if let Some(meta) = chunk.eversilver {
                    ...
                    for choice in chunk.choices {
                        ...
                        if let Some(content) = choice.delta.content.as_ref() {
                            if !content.is_empty() {
                                ...
                                let _ = delta_tx
                                    .send(crate::eversilver::providers::ProviderDelta::TextDelta {
                                        delta: content.clone(),
                                    })
                                    .await;
                            }
                        ...
                        if let Some(reasoning) = choice.delta.reasoning_content.as_ref() {
                            if !reasoning.is_empty() {
                                ...
                                let _ = delta_tx
                                    .send(
                                        crate::eversilver::providers::ProviderDelta::ThinkingDelta {
                            ...
                            for tc in tc_list {
                                ...
                                if !entry.emitted_start {
                                        ...
                                        (entry.id.as_ref(), entry.name.as_ref())
                                    {
                                        ...
                                        let _ = delta_tx
                                            .send(crate::eversilver::providers::ProviderDelta::ToolCallStart {
                                                call_id: id.clone(),
                                        ...
                                        if !entry.arguments.is_empty() {
                                            ...
                                            let _ = delta_tx
                                                .send(crate::eversilver::providers::ProviderDelta::ToolCallArgsDelta {
                                                    call_id: id.clone(),
                                                    delta: buffered,
                                                })
                                                .await;
        ...
        let api_resp = ApiChatResponse {
            ...
            usage: last_usage,
            eversilver: last_eversilver,
        };

        // Dump the aggregated final response (structured, diff-friendly,
        // carries usage + eversilver cache meta from the last chunks).
        ...
        if std::env::var("EVERSILVER_PROMPT_DUMP_DIR").is_ok() {
            ...
            let aggregated = serde_json::json!({
                ...
                "eversilver": api_resp.eversilver.as_ref().map(|m| serde_json::json!({
...
impl Provider for OpenAiCompatibleProvider {
    fn capabilities(&self) -> crate::eversilver::providers::traits::ProviderCapabilities {
        crate::eversilver::providers::traits::ProviderCapabilities {
    ...
    async fn chat(
        ...
        if let Some(tx) = request.stream {
            let native_request = NativeChatRequest {
                ...
                // accounting (and `eversilver.billing.charged_amount_usd`
