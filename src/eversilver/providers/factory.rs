...
//!
//! ```text
//! "eversilver"        → EversilverBackendProvider; model = config.default_model
...
//! ""  / missing      → falls back to "eversilver"
//! ```
//!
//! Unknown slugs and missing-creds configurations produce actionable errors.

use crate::eversilver::config::schema::cloud_providers::AuthStyle;
use crate::eversilver::config::Config;
use crate::eversilver::credentials::AuthService;
use crate::eversilver::providers::compatible::{
    AuthStyle as CompatAuthStyle, OpenAiCompatibleProvider,
};
use crate::eversilver::providers::eversilver_backend::EversilverBackendProvider;
use crate::eversilver::providers::traits::Provider;
use crate::eversilver::providers::ProviderRuntimeOptions;

/// Sentinel meaning "use the Eversilver backend session JWT".
pub const PROVIDER_EVERSILVER: &str = "eversilver";
...
///
/// Returns `"eversilver"` when the workload has no explicit override.
...
pub fn create_chat_provider_from_string(
    ...
    anyhow::bail!(
        "[chat-factory] unrecognised provider string '{}' for role '{}'. \
         Valid forms: eversilver, ollama:<model>, <slug>:<model>. \
         Configured slugs: [{}]",
        p,
        role,
        config
...
fn make_eversilver_backend(config: &Config) -> anyhow::Result<(Box<dyn Provider>, String)> {
    ...
    // `~/.eversilver`, and reads an unrelated (or empty)
    ...
    log::debug!(
        "[providers][chat-factory] building eversilver backend provider model={} state_dir={:?} secrets_encrypt={}",
        model,
    ...
    let model = match model.strip_prefix("hint:") {
        Some("reasoning") => crate::eversilver::config::MODEL_REASONING_V1.to_string(),
        Some("chat") => crate::eversilver::config::MODEL_REASONING_QUICK_V1.to_string(),
        Some("agentic") => crate::eversilver::config::MODEL_AGENTIC_V1.to_string(),
        Some("coding") => crate::eversilver::config::MODEL_CODING_V1.to_string(),
        _ => model,
    };
...
fn make_cloud_provider_by_slug(
    ...
    match entry.auth_style {
        ...
        AuthStyle::EversilverJwt => {
            ...
            log::debug!(
                "[providers][chat-factory] slug='{}' has auth_style=EversilverJwt → routing to eversilver backend",
                slug
            );
...
mod tests {
    use super::*;
    use crate::eversilver::config::schema::cloud_providers::{AuthStyle, CloudProviderCreds};
    use crate::eversilver::config::Config;
    ...
    fn oh_entry(id: &str) -> CloudProviderCreds {
        CloudProviderCreds {
            id: id.to_string(),
            slug: "eversilver".to_string(),
            label: "Eversilver".to_string(),
            endpoint: "https://api.eversilver.ai/v1".to_string(),
            auth_style: AuthStyle::EversilverJwt,
            ..Default::default()
        }
    }
    ...
    fn eversilver_literal() {
        ...
        let (_, model) = create_chat_provider_from_string("reasoning", "eversilver", &config)
            .expect("eversilver literal must build");
    ...
    fn cloud_no_providers_falls_back_to_eversilver() {
        let config = Config::default();
        // "cloud" sentinel still works — routes to eversilver.
    ...
    fn all_workloads_default_to_eversilver() {
        ...
        for role in &[
            ...
            assert_eq!(
                ...
                "eversilver",
                "role={role} must default to eversilver"
            );
        }
    }
    ...
    fn workload_override_respected() {
        ...
        assert_eq!(provider_for_role("reasoning", &config), "eversilver");
    }
    ...
    fn summarization_defaults_to_eversilver_like_memory() {
        let config = Config::default();
        assert_eq!(provider_for_role("memory", &config), "eversilver");
        assert_eq!(provider_for_role("summarization", &config), "eversilver");
    }
    ...
    fn unknown_workload_falls_back_to_eversilver() {
        ...
        assert_eq!(provider_for_role("", &config), "eversilver");
    }
    ...
    fn eversilver_backend_uses_config_path_parent_as_state_dir() {
        ...
        let (_provider, model) = create_chat_provider("reasoning", &config)
            .expect("eversilver backend must build with no cloud_providers");
