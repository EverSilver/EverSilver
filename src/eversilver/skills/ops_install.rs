...
/// it to `<workspace>/.eversilver/skills/<slug>/SKILL.md`.
///
/// Design rationale: eversilver's skill discovery scans
/// `<workspace>/.eversilver/skills/` (plus `~/.eversilver/skills/` and legacy
...
/// agent ecosystem is incompatible with eversilver's skill layout, so we fetch
...
pub async fn install_skill_from_url(
    ...
    // Install to user scope (`~/.eversilver/skills/<slug>`), which `discover_skills`
    // scans unconditionally. Project scope (`<ws>/.eversilver/skills/`) is gated on
    // a `<ws>/.eversilver/trust` marker and would render the install invisible to the
...
pub struct UninstallSkillParams {
    ...
    /// `~/.eversilver/skills/<slug>/`. Retained as `name` for wire-format
...
/// Remove an installed user-scope SKILL.md skill from `~/.eversilver/skills/`.
///
