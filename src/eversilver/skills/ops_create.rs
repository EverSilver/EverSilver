...
///
/// Scope resolution:
/// * [`SkillScope::User`] → `~/.eversilver/skills/`
/// * [`SkillScope::Project`] → `<workspace>/.eversilver/skills/`. Requires the
///   trust marker at `<workspace>/.eversilver/trust` to be present; otherwise
    ...
    let scope_root = match params.scope {
        ...
        SkillScope::Project => {
            if !is_workspace_trusted(workspace_dir) {
                return Err(format!(
                    "workspace {} is not trusted; create {}/.eversilver/trust to enable project-scope skills",
                    workspace_dir.display(),
                    workspace_dir.display(),
                ));
            }
            workspace_dir.join(".eversilver").join("skills")
        }
        SkillScope::Legacy => {
            return Err(
