...
pub enum SkillScope {
    /// Skill shipped with the user's global config (`~/.eversilver/skills/...`).
    User,
    /// Skill shipped with the current workspace (`<ws>/.eversilver/skills/...`).
...
pub struct Skill {
    ...
    pub name: String,
    /// On-disk slug — the directory name under `~/.eversilver/skills/` (user
