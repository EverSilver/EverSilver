...
/// `<workspace>/.eversilver/skills/` instead, but this directory is kept for
...
/// so user-scope skills (`~/.eversilver/skills/`, `~/.agents/skills/`) are
...
///   `~/.eversilver/skills/` and `~/.agents/skills/`.
...
///
/// Looks for `<workspace>/.eversilver/trust`. The marker file's contents are
/// ignored — presence is sufficient.
pub fn is_workspace_trusted(workspace_dir: &Path) -> bool {
    workspace_dir.join(".eversilver").join(TRUST_MARKER).exists()
}
...
fn user_roots(home: &Path) -> Vec<PathBuf> {
    vec![
        home.join(".eversilver").join("skills"),
...
fn project_roots(workspace: &Path) -> Vec<PathBuf> {
    vec![
        workspace.join(".eversilver").join("skills"),
...
/// `.eversilver/trust` marker) and locating the matching entry; this keeps the
