//! `eversilver tree-summarizer` — CLI for the hierarchical summary tree.
//!
//! Ingest content, run summarization jobs, query the tree, and inspect
//! status from the terminal without starting the full app.
//!
//! Usage:
//!   eversilver tree-summarizer ingest  <namespace> [--content <text> | --file <path>] [-v]
//!   eversilver tree-summarizer run     <namespace> [-v]
//!   eversilver tree-summarizer query   <namespace> [<node_id>] [-v]
//!   eversilver tree-summarizer status  <namespace> [-v]
//!   eversilver tree-summarizer rebuild <namespace> [-v]

use anyhow::Result;

/// Entry point for `eversilver tree-summarizer <subcommand>`.
pub(crate) fn run_tree_summarizer_command(args: &[String]) -> Result<()> {
    ...
    match args[0].as_str() {
        ...
        other => Err(anyhow::anyhow!(
            "unknown tree-summarizer subcommand '{other}'. Run `eversilver tree-summarizer --help`."
        )),
    }
}
...
/// `eversilver tree-summarizer ingest <namespace> --content <text>` or `--file <path>`
fn run_ingest(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: eversilver tree-summarizer ingest <namespace> [--content <text>] [--file <path>] [-v]");
        println!();
    ...
    let content = if let Some(ref path) = opts.file {
        ...
        return Err(anyhow::anyhow!(
            "either --content or --file is required. Run `eversilver tree-summarizer ingest --help`."
        ));
    };
    ...
    rt.block_on(async {
        ...
        let outcome = crate::eversilver::tree_summarizer::rpc::tree_summarizer_ingest(
...
/// `eversilver tree-summarizer run <namespace>`
fn run_summarize(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: eversilver tree-summarizer run <namespace> [-v]");
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome =
            crate::eversilver::tree_summarizer::rpc::tree_summarizer_run(&config, namespace)
                .await
                .map_err(anyhow::Error::msg)?;
...
/// `eversilver tree-summarizer query <namespace> [<node_id>]`
fn run_query(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!(
            "Usage: eversilver tree-summarizer query <namespace> [<node_id>] [--node-id <id>] [-v]"
        );
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome = crate::eversilver::tree_summarizer::rpc::tree_summarizer_query(
...
/// `eversilver tree-summarizer status <namespace>`
fn run_status(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: eversilver tree-summarizer status <namespace> [-v]");
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome =
            crate::eversilver::tree_summarizer::rpc::tree_summarizer_status(&config, namespace)
                .await
                .map_err(anyhow::Error::msg)?;
...
/// `eversilver tree-summarizer rebuild <namespace>`
fn run_rebuild(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: eversilver tree-summarizer rebuild <namespace> [-v]");
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome =
            crate::eversilver::tree_summarizer::rpc::tree_summarizer_rebuild(&config, namespace)
                .await
                .map_err(anyhow::Error::msg)?;
...
async fn load_config() -> Result<crate::eversilver::config::Config> {
    let mut config = crate::eversilver::config::Config::load_or_init()
        .await
        .unwrap_or_default();
...
fn print_help() {
    println!("eversilver tree-summarizer — hierarchical summary tree\n");
    println!("Usage:");
    println!(
        "  eversilver tree-summarizer ingest  <namespace> [--content <text>] [--file <path>] [-v]"
    );
    println!("  eversilver tree-summarizer run     <namespace> [-v]");
    println!("  eversilver tree-summarizer query   <namespace> [<node_id>] [-v]");
    println!("  eversilver tree-summarizer status  <namespace> [-v]");
    println!("  eversilver tree-summarizer rebuild <namespace> [-v]");
    println!();
    println!("Subcommands:");
    ...
    println!();
    println!("Examples:");
    println!("  eversilver tree-summarizer ingest my-ns --content 'Some raw data to summarize'");
    println!("  eversilver tree-summarizer ingest my-ns --file notes.txt");
    println!("  cat journal.md | eversilver tree-summarizer ingest my-ns --file -");
    println!("  eversilver tree-summarizer run my-ns");
    println!("  eversilver tree-summarizer query my-ns root");
    println!("  eversilver tree-summarizer query my-ns 2024/03/15");
    println!("  eversilver tree-summarizer status my-ns");
}
