...
/// `backend-eversilver/src/middlewares/errorHandler.ts`). When the body parses
...
pub(crate) fn extract_error_detail(body: &str, max_bytes: usize) -> String {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(body) {
        if let Some(msg) = v.get("error").and_then(|e| e.as_str()) {
            ...
            if !trimmed.is_empty() {
                return crate::eversilver::util::truncate_at_byte_boundary(trimmed, max_bytes);
            }
        }
    }
    crate::eversilver::util::truncate_at_byte_boundary(body, max_bytes)
}
...
pub fn build_client(config: &crate::eversilver::config::Config) -> Option<Arc<IntegrationClient>> {
