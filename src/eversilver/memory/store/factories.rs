...
use crate::eversilver::embeddings::{
...
use crate::eversilver::memory::store::agentmemory::AgentMemoryBackend;
use crate::eversilver::memory::store::unified::UnifiedMemory;
use crate::eversilver::memory::traits::Memory;
...
///
/// Delegates to [`crate::eversilver::local_ai::ollama_base_url`] so the probe
...
fn ollama_base_url_for_probe() -> String {
    crate::eversilver::local_ai::ollama_base_url()
}
    ...
    if is_agentmemory_backend(&config.backend) {
        log::info!(
            ...
            config
                .agentmemory_url
                .as_deref()
                .unwrap_or(crate::eversilver::memory::store::agentmemory_default_url()),
        );
...
mod tests {
    ...
    impl EnvGuard {
        fn set(value: &str) -> Self {
            let lock = crate::eversilver::local_ai::local_ai_test_guard();
    ...
    fn embedding_settings_local_overrides_memory_config() {
        ...
        assert_eq!(
            dims,
            crate::eversilver::embeddings::DEFAULT_OLLAMA_DIMENSIONS,
    ...
    fn embedding_settings_local_with_empty_model_uses_default() {
        ...
        assert_eq!(
            model,
            crate::eversilver::embeddings::DEFAULT_OLLAMA_MODEL,
    ...
    fn ollama_health_gate_reports_at_most_once_per_process() {
        let _lock = crate::eversilver::local_ai::local_ai_test_guard();
        reset_health_gate_for_test();
