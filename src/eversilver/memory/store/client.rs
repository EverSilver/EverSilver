...
use crate::eversilver::embeddings::{self, EmbeddingProvider};
use crate::eversilver::memory::ingestion::queue as ingestion_queue;
use crate::eversilver::memory::ingestion::{
    IngestionJob, IngestionQueue, IngestionState, MemoryIngestionConfig, MemoryIngestionRequest,
    MemoryIngestionResult,
};
use crate::eversilver::memory::store::types::{
    NamespaceDocumentInput, NamespaceMemoryHit, NamespaceRetrievalContext,
};
use crate::eversilver::memory::store::unified::UnifiedMemory;
...
/// [`MemoryConfig.embedding_provider`](crate::eversilver::config::MemoryConfig)
/// resolves to — cloud (Eversilver backend, the default returned by
/// [`crate::eversilver::embeddings::default_embedding_provider`]) or local Ollama
...
/// store via [`crate::eversilver::memory::create_memory_with_storage_and_routes`]
/// (or [`crate::eversilver::memory::create_memory_with_local_ai`]) with the
...
impl MemoryClient {
    ...
    /// [`crate::eversilver::memory::store::unified::profile::profile_upsert`].
    ///
    ...
    pub fn memory_handle(&self) -> Arc<dyn crate::eversilver::memory::Memory> {
        Arc::clone(&self.inner) as Arc<dyn crate::eversilver::memory::Memory>
    }

    /// Create a new local memory client using the default `.eversilver` directory.
    ///
    /// # Errors
    ///
    ...
    pub fn new_local() -> Result<Self, String> {
        let workspace_dir = crate::eversilver::config::default_root_eversilver_dir()
    ...
    /// Used by the `eversilver.memory_ingestion_status` RPC handler.
