...
use crate::eversilver::memory::{MemoryClient, MemoryClientRef};
...
/// Initialise using the default `~/.eversilver/workspace` directory.
///
...
/// would pin the singleton to `~/.eversilver/workspace`, causing every
...
/// fallback: a fallback would pin the global to `~/.eversilver/workspace` on
