...
//! - `eversilver.memory_tree_ingest` — one unified ingest. Caller supplies
//!   `source_kind` + generic JSON `payload` (adapter-specific). Internally
//!   dispatches to chat / email / document canonicalisers.
//! - `eversilver.memory_tree_list_chunks` — listing with filters.
//! - `eversilver.memory_tree_get_chunk` — single chunk fetch.
...
use crate::eversilver::memory::tree::ingest::{
    ingest_chat as do_ingest_chat, ingest_document as do_ingest_document,
    ingest_email as do_ingest_email, IngestResult,
};
use crate::eversilver::memory::tree::store::{self, ListChunksQuery};
use crate::eversilver::memory::tree::types::{Chunk, SourceKind};
...
pub async fn trigger_digest_rpc(
    ...
    use crate::eversilver::memory::tree::jobs;
...
mod tests {
    use super::*;
    use crate::eversilver::memory::tree::jobs::store::count_total;
