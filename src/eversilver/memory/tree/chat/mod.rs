...
//!
//! The existing top-level [`crate::eversilver::providers::Provider`] trait
...
use crate::eversilver::config::{Config, LlmBackend, DEFAULT_CLOUD_LLM_MODEL};
...
pub fn build_chat_provider(
    ...
    if let Some(routed_model) = config.workload_local_model("memory") {
        ...
        // inner EversilverBackendProvider falls back to `~/.eversilver`
