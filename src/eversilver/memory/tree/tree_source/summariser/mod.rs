...
///   routed through the Eversilver backend's `cloud_llm_model`
...
pub fn build_summariser(config: &Config) -> Arc<dyn Summariser> {
    use crate::eversilver::config::DEFAULT_CLOUD_LLM_MODEL;
    use crate::eversilver::memory::tree::chat::{build_chat_provider, ChatConsumer};
