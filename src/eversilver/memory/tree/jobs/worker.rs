...
//! [`crate::eversilver::scheduler_gate`] — its global single-slot
...
use crate::eversilver::memory::tree::jobs::handlers;
use crate::eversilver::memory::tree::jobs::redact::scrub_for_log;
use crate::eversilver::memory::tree::jobs::store::{
    claim_next, mark_deferred, mark_done, mark_failed, recover_stale_locks,
    DEFAULT_LOCK_DURATION_MS,
};
use crate::eversilver::memory::tree::jobs::types::JobOutcome;
...
pub async fn run_once(config: &Config) -> Result<bool> {
    ...
    let gate_permit = crate::eversilver::scheduler_gate::wait_for_capacity().await;
