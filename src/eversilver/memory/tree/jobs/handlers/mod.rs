...
use crate::eversilver::memory::tree::content_store::{
    self as content_store, read as content_read, tags as content_tags,
};
use crate::eversilver::memory::tree::jobs::store;
use crate::eversilver::memory::tree::jobs::types::{
...
use crate::eversilver::memory::tree::score;
use crate::eversilver::memory::tree::score::embed::{build_embedder_from_config, pack_checked};
use crate::eversilver::memory::tree::score::extract::build_summary_extractor;
use crate::eversilver::memory::tree::score::store as score_store;
use crate::eversilver::memory::tree::store as chunk_store;
use crate::eversilver::memory::tree::tree_global::digest::{self, DigestOutcome};
use crate::eversilver::memory::tree::tree_source::{
    build_summariser, get_or_create_source_tree, LabelStrategy, LeafRef,
};
use crate::eversilver::memory::tree::tree_topic::curator;
        ...
        if let Some(content_path) = chunk_store::get_chunk_content_path(config, &chunk.id)? {
            ...
            if let Err(e) = content_tags::update_chunk_tags(&abs_path, &obsidian_tags) {
                log::warn!(
                    ...
                    chunk.id,
                    crate::eversilver::memory::tree::util::redact::redact(&content_path),
                );
...
async fn handle_append_buffer(config: &Config, job: &Job) -> Result<JobOutcome> {
    use crate::eversilver::memory::tree::tree_source::bucket_seal::should_seal;
    use crate::eversilver::memory::tree::tree_source::store as src_store;
...
async fn handle_seal(config: &Config, job: &Job) -> Result<JobOutcome> {
    use crate::eversilver::memory::tree::tree_source::bucket_seal::{seal_one_level, should_seal};
    use crate::eversilver::memory::tree::tree_source::store as src_store;
    use crate::eversilver::memory::tree::tree_source::types::TreeKind;
...
async fn handle_topic_route(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    let node_id: String = match &payload.node {
        ...
        NodeRef::Summary { summary_id } => {
            if crate::eversilver::memory::tree::tree_source::store::get_summary(config, summary_id)?
                .is_none()
            {
                log::warn!(
    ...
    for entity_id in entity_ids {
        ...
        if let Some(tree) = crate::eversilver::memory::tree::tree_source::store::get_tree_by_scope(
            config,
            crate::eversilver::memory::tree::tree_source::types::TreeKind::Topic,
            &entity_id,
        )? {
...
async fn handle_flush_stale(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    let age_secs = payload
        .max_age_secs
        .unwrap_or(crate::eversilver::memory::tree::tree_source::types::DEFAULT_FLUSH_AGE_SECS);
    let cutoff = chrono::Utc::now() - chrono::Duration::seconds(age_secs);
    let buffers =
        crate::eversilver::memory::tree::tree_source::store::list_stale_buffers(config, cutoff)?;
    for buf in buffers {
        let seal = SealPayload {
...
mod tests {
    use super::*;
    use crate::eversilver::memory::tree::content_store;
    use crate::eversilver::memory::tree::jobs::store::{count_by_status, count_total};
    use crate::eversilver::memory::tree::jobs::types::JobStatus;
    use crate::eversilver::memory::tree::store::with_connection;
    use crate::eversilver::memory::tree::tree_source::bucket_seal::{append_leaf_deferred, LeafRef};
    use crate::eversilver::memory::tree::tree_source::registry::get_or_create_source_tree;
    use crate::eversilver::memory::tree::tree_source::store as src_store;
    use chrono::TimeZone;
    use rusqlite::params;
    ...
    async fn seed_source_tree_ready_to_seal(
        cfg: &Config,
    ) -> crate::eversilver::memory::tree::tree_source::types::Tree {
        use crate::eversilver::memory::tree::store::upsert_chunks;
        use crate::eversilver::memory::tree::types::{
        ...
        with_connection(cfg, |conn| {
            let tx = conn.unchecked_transaction()?;
            crate::eversilver::memory::tree::store::upsert_staged_chunks_tx(&tx, &staged)?;
            tx.commit()?;
            Ok(())
        })
        .unwrap();
    ...
    async fn topic_tree_seal_handler_does_not_enqueue_topic_route() {
        ...
        let topic_tree =
            crate::eversilver::memory::tree::tree_topic::registry::get_or_create_topic_tree(
                &cfg,
    ...
    async fn handle_append_buffer_with_summary_payload_pushes_into_topic_tree() {
        ...
        use crate::eversilver::memory::tree::store::upsert_chunks;
        use crate::eversilver::memory::tree::tree_source::bucket_seal::seal_one_level;
        use crate::eversilver::memory::tree::types::{
            chunk_id, Chunk, Metadata, SourceKind, SourceRef,
        };
        let ts = chrono::Utc.timestamp_millis_opt(1_700_000_000_000).unwrap();
        ...
        let summary_id = seal_one_level(
            ...
            &buf,
            summariser.as_ref(),
            &crate::eversilver::memory::tree::tree_source::bucket_seal::LabelStrategy::Empty,
