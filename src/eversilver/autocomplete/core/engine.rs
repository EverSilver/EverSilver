...
impl AutocompleteEngine {
    ...
    pub async fn start(
        ...
        PRECOMPILE_ONCE.call_once(|| {
            crate::eversilver::accessibility::precompile_helper_background();
        });
                ...
                if last_refresh.elapsed() >= Duration::from_millis(current_debounce_ms) {
                    ...
                    match refresh_result {
                        Ok(Ok(Err(err))) => {
                            ...
                            if is_perm_denied {
                                crate::eversilver::accessibility::mark_system_events_denied();
                            }
                            ...
                            if should_notify && is_perm_denied {
                                ...
                                let app_lower = engine
                                    ...
                                    .unwrap_or_default()
                                    .to_lowercase();
                                if !app_lower.contains("eversilver") {
                                    show_overflow_badge(
                                        "error",
                                        None,
                                        Some(&err),
    ...
    pub async fn accept(
        ...
        // Dual-write: KV (UI list) + local docs (semantic search).
        {
            ...
            tokio::spawn(async move {
                crate::eversilver::autocomplete::history::save_accepted_completion(
                    &ctx,
                    &sug,
                    app.as_deref(),
                )
                .await;
                crate::eversilver::autocomplete::history::save_completion_to_local_docs(
                    &ctx,
                    &sug,
                    app.as_deref(),
                )
    ...
    async fn refresh(&self, context_override: Option<String>) -> Result<(), String> {
        ...
        if !is_in_app && app_lower.contains("eversilver") {
            let mut state = self.inner.lock().await;
            state.app_name = focused.app_name;
            state.phase = "idle".to_string();
            state.last_error = None;
        ...
        let (relevant_examples, recent_examples) = if is_in_app {
            ...
            let relevant_future =
                crate::eversilver::autocomplete::history::query_relevant_examples(&context, 4);
            let recent_future = crate::eversilver::autocomplete::history::load_recent_examples(4);
...
pub async fn start_if_enabled(app_config: &Config) {
    ...
    crate::eversilver::accessibility::clear_automation_denial();
