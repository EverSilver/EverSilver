...
async fn execute_and_persist_job(
    ...
    publish_global(DomainEvent::CronJobCompleted {
        job_id: job.id.clone(),
        success,
        output: crate::eversilver::util::truncate_with_ellipsis(&output, 512),
    });
    let failure_message =
        (!success).then(|| crate::eversilver::util::truncate_with_ellipsis(&output, 256));
            ...
            crate::eversilver::agent::harness::definition::AgentDefinitionRegistry::global()
        {
            if let Some(def) = registry.get(agent_id) {
                ...
                let fallback_model = effective
                    .default_model
                    .clone()
                    .unwrap_or_else(|| crate::eversilver::config::DEFAULT_MODEL.to_string());
                let resolved_model = match &def.model {
                    ModelSpec::Hint(workload) => {
                        match crate::eversilver::providers::create_chat_provider(
                            workload, &effective,
                        ) {
                            Ok((_, m)) => {
                                tracing::debug!(
...
fn push_cron_alert(config: &Config, job: &CronJob, output: &str) {
    use crate::eversilver::notifications::store as notif_store;
    use crate::eversilver::notifications::types::{IntegrationNotification, NotificationStatus};

    let name = job.name.as_deref().unwrap_or("Cron job");
    let truncated = crate::eversilver::util::truncate_with_ellipsis(output, 512);
