...
use crate::eversilver::accessibility::{parse_foreground_output, AppContext};
use crate::eversilver::config::{Config, ScreenIntelligenceConfig};
use crate::eversilver::embeddings::NoopEmbedding;
use crate::eversilver::memory::store::UnifiedMemory;
...
fn screen_intelligence_env_lock() -> std::sync::MutexGuard<'static, ()> {
    match crate::eversilver::config::TEST_ENV_LOCK.lock() {
        Ok(guard) => guard,
