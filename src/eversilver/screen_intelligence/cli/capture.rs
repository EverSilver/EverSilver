...
/// `eversilver screen-intelligence capture` — take a single screenshot and print info.
pub(super) fn run_capture(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: eversilver screen-intelligence capture [--keep] [-v]");
        println!();
    ...
    rt.block_on(async {
        ...
        if result.ok {
            ...
            if opts.keep {
                ...
                match crate::eversilver::screen_intelligence::AccessibilityEngine::save_capture_test_result(
...
/// `eversilver screen-intelligence vision` — inspect recent vision summaries.
pub(super) fn run_vision(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: eversilver screen-intelligence vision [--limit <n>] [-v]");
        println!();
    ...
    rt.block_on(async {
        ...
        if result.summaries.is_empty() {
            ...
            eprintln!("  Start a session first: eversilver screen-intelligence start");
        } else {
