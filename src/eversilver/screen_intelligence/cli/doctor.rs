//! `eversilver screen-intelligence doctor` — diagnostic readiness check.
...
pub(super) fn run_doctor(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: eversilver screen-intelligence doctor [-v]");
        println!();
    ...
    rt.block_on(async {
        ...
        let doctor_json =
            crate::eversilver::screen_intelligence::rpc::accessibility_doctor_cli_json()
                .await
        ...
        // Vision config check.
        let config = crate::eversilver::config::Config::load_or_init().await.ok();
