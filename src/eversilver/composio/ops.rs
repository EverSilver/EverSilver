...
///
/// The shared [`crate::eversilver::integrations::IntegrationClient`]
...
/// `eversilver.composio_get_user_profile` — fetch a normalized user
...
/// `eversilver.composio_refresh_all_identities` — re-fetch the user
...
/// `eversilver.composio_sync` — run a sync pass for a connected account
...
use crate::eversilver::context::prompt::{ConnectedIntegration, ConnectedIntegrationTool};
...
/// resolves to the per-user eversilver dir).
...
pub async fn composio_get_mode(config: &Config) -> OpResult<RpcOutcome<serde_json::Value>> {
    ...
    let key_present = crate::eversilver::credentials::get_composio_api_key(config)
...
pub async fn composio_set_api_key(
    ...
    crate::eversilver::credentials::store_composio_api_key(config, trimmed)
        .await
    ...
    let mode_log = if activate_direct {
        ...
        // all observe it.
        let mut cfg_mut = crate::eversilver::config::rpc::load_config_with_timeout()
            .await
            .map_err(|e| format!("[composio-direct] reload config failed: {e}"))?;
        cfg_mut.composio.mode = crate::eversilver::config::schema::COMPOSIO_MODE_DIRECT.into();
        cfg_mut
            .save()
            .await
...
pub async fn composio_clear_api_key(config: &Config) -> OpResult<RpcOutcome<serde_json::Value>> {
    ...
    crate::eversilver::credentials::clear_composio_api_key(config)
        .await
    ...
    let mut cfg_mut = crate::eversilver::config::rpc::load_config_with_timeout()
        .await
        .map_err(|e| format!("[composio-direct] reload config failed: {e}"))?;
    cfg_mut.composio.mode = crate::eversilver::config::schema::COMPOSIO_MODE_BACKEND.into();
    cfg_mut
        .save()
        .await
        .map_err(|e| format!("[composio-direct] save config failed: {e}"))?;
