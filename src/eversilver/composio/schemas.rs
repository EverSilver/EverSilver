...
//! `eversilver.composio_*`:
//!   - `composio.list_toolkits`       → `eversilver.composio_list_toolkits`
//!   - `composio.list_connections`    → `eversilver.composio_list_connections`
//!   - `composio.authorize`           → `eversilver.composio_authorize`
//!   - `composio.delete_connection`   → `eversilver.composio_delete_connection`
//!   - `composio.list_tools`          → `eversilver.composio_list_tools`
//!   - `composio.execute`             → `eversilver.composio_execute`
//!   - `composio.list_github_repos`   → `eversilver.composio_list_github_repos`
//!   - `composio.create_trigger`      → `eversilver.composio_create_trigger`
//!   - `composio.get_user_profile`    → `eversilver.composio_get_user_profile`
//!   - `composio.refresh_all_identities` → `eversilver.composio_refresh_all_identities`
//!   - `composio.sync`                → `eversilver.composio_sync`
...
fn handle_set_user_scopes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let memory = match crate::eversilver::memory::global::client_if_ready() {
            Some(m) => m,
            None => {
                tracing::error!(
                    method = "composio.set_user_scopes",
