...
fn fake_config_arc() -> Arc<crate::eversilver::config::Config> {
...
async fn authorize_tool_execute_rejects_missing_toolkit() {
    ...
    let txt = result
        ...
        .filter_map(|c| match c {
            crate::eversilver::tools::traits::ToolContent::Text { text } => Some(text.clone()),
            _ => None,
        })
        .collect::<Vec<_>>()
        .join(" ");
...
fn agent_tools_register_when_backend_signed_in() {
    ...
    let mut config = crate::eversilver::config::Config::default();
    config.config_path = tmp.path().join("config.toml");
    crate::eversilver::credentials::AuthService::from_config(&config)
        .store_provider_token(
            crate::eversilver::credentials::APP_SESSION_PROVIDER,
            crate::eversilver::credentials::DEFAULT_AUTH_PROFILE_NAME,
            "test-token",
...
async fn sandbox_read_only_blocks_write_scope_action() {
    let t = ComposioExecuteTool::new(fake_config_arc());
    let result =
        crate::eversilver::agent::harness::with_current_sandbox_mode(SandboxMode::ReadOnly, async {
...
async fn sandbox_sandboxed_mode_does_not_trigger_readonly_gate() {
    ...
    let t = ComposioExecuteTool::new(Arc::new(config));
    let result = crate::eversilver::agent::harness::with_current_sandbox_mode(
...
fn render_tools_markdown_groups_by_toolkit_and_drops_schemas() {
    use crate::eversilver::composio::types::{
...
fn render_tools_markdown_handles_empty_response() {
    use crate::eversilver::composio::types::ComposioToolsResponse;
...
fn direct_mode_config() -> crate::eversilver::config::Config {
...
fn execute_tool_resolves_to_direct_kind_when_mode_is_direct() {
    ...
    let kind = crate::eversilver::composio::client::create_composio_client(&config)
        .expect("direct mode with inline api_key must resolve");
    assert_eq!(
        kind.mode(),
        crate::eversilver::config::schema::COMPOSIO_MODE_DIRECT,
...
fn execute_tool_resolves_to_backend_kind_when_mode_is_backend() {
    ...
    assert_eq!(
        kind.mode(),
        crate::eversilver::config::schema::COMPOSIO_MODE_BACKEND,
...
async fn execute_tool_per_call_factory_means_no_baked_client() {
    ...
    // per call which reads from `~/.eversilver/config.toml` (or the
