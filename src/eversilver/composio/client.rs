//! Thin HTTP wrapper over the eversilver backend's
//! `/agent-integrations/composio/*` routes.
//!
//! All calls go through the shared
//! [`crate::eversilver::integrations::IntegrationClient`] so they inherit
...
use crate::eversilver::integrations::IntegrationClient;
...
impl ComposioClient {
    ...
    async fn raw_delete<T: serde::de::DeserializeOwned>(&self, path: &str) -> Result<T> {
        ...
        if !status.is_success() {
            ...
            let detail = crate::eversilver::integrations::client::extract_error_detail(
                &body_text,
                crate::eversilver::integrations::client::MAX_ERROR_BODY_LEN,
            );
            ...
            let logged_body =
                crate::eversilver::integrations::client::extract_error_detail(&body_text, 300);
            tracing::debug!(
...
/// [`crate::eversilver::agent::harness::subagent_runner::user_is_signed_in_to_composio`]
/// (probe) instead.
///
...
/// [`crate::eversilver::integrations::build_client`]. The only reason
...
pub(super) fn build_composio_client(
    ...
    let inner = crate::eversilver::integrations::build_client(config)?;
    Some(ComposioClient::new(inner))
}

// ── Direct-mode factory ─────────────────────────────────────────────
//
// Mirrors `src/eversilver/embeddings/factory.rs` so anyone reading both
...
use crate::eversilver::config::schema::{COMPOSIO_MODE_BACKEND, COMPOSIO_MODE_DIRECT};
...
pub enum ComposioClientKind {
    ...
    Direct(Arc<crate::eversilver::tools::ComposioTool>),
}
...
///   [`crate::eversilver::credentials::COMPOSIO_DIRECT_PROVIDER`]
...
pub fn create_composio_client(
    ...
    match mode {
        ...
        MODE_DIRECT_PAT => {
            ...
            let stored = crate::eversilver::credentials::get_composio_api_key(config)
            ...
            // context.
            let security = Arc::new(crate::eversilver::security::SecurityPolicy::default());
            let tool = crate::eversilver::tools::ComposioTool::new(
                &api_key,
...
/// [`crate::eversilver::tools::ComposioTool::get_connection_url`] and
...
/// [`crate::eversilver::tools::ComposioTool::list_connected_accounts`])
...
/// [`crate::eversilver::tools::ComposioTool::list_tool_schemas_v3`] and
