...
fn config_with_backend(tmp: &tempfile::TempDir, base: String) -> Config {
    ...
    c.config_path = tmp.path().join("config.toml");
    c.api_url = Some(base);
    crate::eversilver::credentials::AuthService::from_config(&c)
...
fn direct_mode_config(tmp: &tempfile::TempDir) -> Config {
    ...
    c.workspace_dir = tmp.path().join("workspace");
    c.config_path = tmp.path().join("config.toml");
    c.composio.mode = crate::eversilver::config::schema::COMPOSIO_MODE_DIRECT.into();
    crate::eversilver::credentials::AuthService::from_config(&c)
        .store_provider_token(
            crate::eversilver::credentials::ops::COMPOSIO_DIRECT_PROVIDER,
            crate::eversilver::credentials::DEFAULT_AUTH_PROFILE_NAME,
            "ck_test_direct_key",
            std::collections::HashMap::new(),
            true,
