//! Agent-facing tools that proxy through the eversilver backend's
...
//! `Tool` trait plumbing in [`crate::eversilver::tools`].
//!
...
use crate::eversilver::tools::traits::{
...
pub struct ComposioListToolkitsTool {
    ...
    /// [`crate::eversilver::config::ComposioConfig::mode`] toggle is
...
pub struct ComposioListToolsTool {
    ...
    /// [`crate::eversilver::composio::ops::composio_execute`] and avoids
...
pub struct ComposioExecuteTool {
    ...
    /// [`crate::eversilver::composio::ops::composio_execute`]. See
    /// issue #1710.
    config: Arc<Config>,
}
...
pub fn all_composio_agent_tools(config: &crate::eversilver::config::Config) -> Vec<Box<dyn Tool>> {
    ...
    if !crate::eversilver::agent::harness::subagent_runner::user_is_signed_in_to_composio(config) {
        tracing::debug!(
