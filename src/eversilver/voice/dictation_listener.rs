...
use crate::eversilver::voice::hotkey::{self, ActivationMode, HotkeyEvent};
...
pub async fn start_if_enabled(config: &Config) {
    ...
    let mode = match config.dictation.activation_mode {
        crate::eversilver::config::DictationActivationMode::Push => ActivationMode::Push,
        crate::eversilver::config::DictationActivationMode::Toggle => ActivationMode::Tap,
    };
