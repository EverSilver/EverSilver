...
fn handle_voice_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::eversilver::voice::voice_status(&config).await?)
    })
}
...
fn handle_voice_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::eversilver::voice::voice_transcribe(
                &config,
                &p.audio_path,
                p.context.as_deref(),
                p.skip_cleanup,
...
fn handle_voice_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::eversilver::voice::voice_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
                p.context.as_deref(),
...
fn handle_voice_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::eversilver::voice::voice_tts(&config, &p.text, p.output_path.as_deref()).await?,
        )
    })
}
...
fn handle_voice_reply_synthesize(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let voice = p
            ...
            .unwrap_or_else(|| {
                if provider_name == "piper" {
                    crate::eversilver::voice::DEFAULT_PIPER_VOICE.to_string()
                } else {
                    String::new()
                }
            });
        ...
        let provider =
            crate::eversilver::voice::create_tts_provider(&provider_name, &voice, &config)
...
fn handle_voice_cloud_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let opts = crate::eversilver::voice::cloud_transcribe::CloudTranscribeOptions {
            model: p.model,
            language: p.language,
        ...
        to_json(
            crate::eversilver::voice::cloud_transcribe::transcribe_cloud(
                &config,
                &p.audio_base64,
                &opts,
            )
...
fn handle_voice_stt_dispatch(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let provider =
            crate::eversilver::voice::create_stt_provider(&provider_name, &model, &config)
                .map_err(|e| e.to_string())?;
...
fn effective_stt_provider(config: &crate::eversilver::config::Config) -> String {
...
fn effective_tts_provider(config: &crate::eversilver::config::Config) -> String {
...
fn handle_voice_server_start(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        use crate::eversilver::voice::hotkey::ActivationMode;
        use crate::eversilver::voice::server::{global_server, VoiceServerConfig};
        ...
        let activation_mode = match params.get("activation_mode").and_then(|v| v.as_str()) {
            ...
            None => match config.voice_server.activation_mode {
                crate::eversilver::config::VoiceActivationMode::Push => ActivationMode::Push,
                crate::eversilver::config::VoiceActivationMode::Tap => ActivationMode::Tap,
            },
        };
        ...
        if let Some(existing) = crate::eversilver::voice::server::try_global_server() {
            let existing_status = existing.status().await;
            if existing_status.state != crate::eversilver::voice::server::ServerState::Stopped {
        ...
        if let Some(s) = crate::eversilver::voice::server::try_global_server() {
...
fn handle_voice_server_stop(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        if let Some(server) = crate::eversilver::voice::server::try_global_server() {
            server.stop().await;
            tokio::time::sleep(std::time::Duration::from_millis(200)).await;
            ...
            let status = crate::eversilver::voice::server::VoiceServerStatus {
                state: crate::eversilver::voice::server::ServerState::Stopped,
                hotkey: String::new(),
                activation_mode: crate::eversilver::voice::hotkey::ActivationMode::Push,
...
fn handle_overlay_stt_notify(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        use crate::eversilver::voice::dictation_listener::{
