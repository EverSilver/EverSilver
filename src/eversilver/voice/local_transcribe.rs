...
//! [`crate::eversilver::local_ai::paths::resolve_whisper_binary`] — kept in
...
//! [`crate::eversilver::local_ai::install_whisper`] which streams the
//! GGML model file (`ggml-<size>.bin`) into
//! `~/.eversilver/bin/whisper/` via a `.part` file + atomic rename, plus
...
use crate::eversilver::local_ai::paths::resolve_whisper_binary_with_config;
...
pub async fn transcribe_whisper(
    ...
    let model_path =
        crate::eversilver::local_ai::paths::resolve_stt_model_path_by_id(&model_id, config)
