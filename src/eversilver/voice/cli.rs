...
//!
//! Handles the `eversilver voice` / `eversilver dictate` subcommand which runs a
...
use crate::eversilver::voice::server::{run_standalone, VoiceServerConfig};

/// Parse and execute the `eversilver voice` / `eversilver dictate` subcommand.
///
/// Supported flags:
...
pub(crate) fn run_standalone_subcommand(args: &[String]) -> Result<()> {
    ...
    rt.block_on(async {
        let mut config = match crate::eversilver::config::Config::load_or_init().await {
            Ok(cfg) => cfg,
            Err(e) => {
...
fn print_help() {
    println!("Usage: eversilver voice [--hotkey <combo>] [--mode <tap|push>] [--skip-cleanup] [-v]");
    println!();
