...
fn core_dispatch_routes_install_to_mock_when_env_set() {
    ...
    let status = crate::eversilver::service::install(&cfg).expect("core::install via mock");
    std::env::remove_var("EVERSILVER_SERVICE_MOCK");
...
fn core_dispatch_routes_status_to_mock_when_env_set() {
    ...
    std::env::set_var("EVERSILVER_SERVICE_MOCK", "1");
    let status = crate::eversilver::service::status(&cfg).expect("core::status via mock");
    std::env::remove_var("EVERSILVER_SERVICE_MOCK");
