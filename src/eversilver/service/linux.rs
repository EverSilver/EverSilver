...
pub(crate) fn install(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "eversilver.service"]));
    Ok(())
}
...
pub(crate) fn start(config: &Config) -> Result<ServiceStatus> {
    if !is_service_enabled_linux()? {
        ...
        let _ =
            run_checked(Command::new("systemctl").args(["--user", "enable", "eversilver.service"]));
    } else {
    ...
    let start_result =
        run_checked(Command::new("systemctl").args(["--user", "start", "eversilver.service"]));
...
pub(crate) fn stop(_config: &Config) -> Result<()> {
    let _ = run_checked(Command::new("systemctl").args(["--user", "stop", "eversilver.service"]));
    Ok(())
}

pub(crate) fn status(config: &Config) -> Result<ServiceStatus> {
    let out =
        run_capture(Command::new("systemctl").args(["--user", "is-active", "eversilver.service"]))
    ...
    Ok(ServiceStatus {
        ...
        label: "eversilver.service".to_string(),
        details: None,
    })
}
...
pub(crate) fn linux_service_file(config: &Config) -> Result<PathBuf> {
    ...
    Ok(config_dir
        ...
        .join("systemd")
        .join("user")
        .join("eversilver.service"))
}

fn is_service_enabled_linux() -> Result<bool> {
    let result = Command::new("systemctl")
        .args(["--user", "is-enabled", "eversilver.service"])
        .output();
...
mod tests {
    ...
    fn linux_service_file_uses_config_dir() {
        ...
        assert!(path.ends_with(".config/systemd/user/eversilver.service"));
    }
}
