...
pub(crate) const SERVICE_LABEL: &str = "com.eversilver.core";
pub(crate) const LEGACY_SERVICE_LABEL: &str = "com.eversilver.daemon";
pub(crate) const LEGACY_APP_LABEL: &str = "com.eversilver.app";
...
pub(crate) fn resolve_daemon_executable() -> Result<PathBuf> {
    ...
    for dir in search_dirs.drain(..) {
        ...
        for entry in entries.flatten() {
            ...
            #[cfg(windows)]
            let matches = name.starts_with("eversilver-core-")
                || name.eq_ignore_ascii_case("eversilver-core.exe");
            #[cfg(not(windows))]
            let matches = name.starts_with("eversilver-core-") || name == "eversilver-core";
