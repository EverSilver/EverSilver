...
// in `{data_dir}/eversilver/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`{data_dir}/eversilver/.secret_key`)
    key_path: PathBuf,
                ...
                #[cfg(windows)]
                {
                    msg.push_str(
                        "\n\nThis is often caused by incorrect file permissions on Windows. \
                         Try repairing ACLs on the .eversilver directory:\n\
                         icacls \"%USERPROFILE%\\.eversilver\" /reset /t /c\n\
                         icacls \"%USERPROFILE%\\.eversilver\\.secret_key\" /reset /c",
                    );
                }
                msg
            })?;
