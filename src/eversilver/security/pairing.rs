...
impl PairingGuard {
    ...
    pub fn new(require_pairing: bool, existing_tokens: &[String]) -> (Self, Option<String>) {
        ...
        log::info!(
            "[eversilver:pairing] Guard created: require_pairing={}, existing_tokens={}, code_generated={}",
            require_pairing,
            tokens.len(),
            code.is_some()
        );
    ...
    fn try_pair_blocking(&self, code: &str) -> Result<Option<String>, u64> {
            ...
            if let (count, Some(locked_at)) = &*attempts {
                if *count >= MAX_PAIR_ATTEMPTS {
                    ...
                    if elapsed < PAIR_LOCKOUT_SECS {
                        log::warn!(
                            "[eversilver:pairing] Pairing locked out: {} failed attempts, {}s remaining",
                            count,
            ...
            if let Some(ref expected) = *pairing_code {
                if constant_time_eq(code.trim(), expected.trim()) {
                    ...
                    *pairing_code = None;
                    ...
                    log::info!("[eversilver:pairing] Pairing successful, token issued");
        ...
        // Increment failed attempts
        {
            ...
            attempts.0 += 1;
            log::warn!(
                "[eversilver:pairing] Pairing attempt failed ({}/{})",
                attempts.0,
                MAX_PAIR_ATTEMPTS
            );
            if attempts.0 >= MAX_PAIR_ATTEMPTS {
                attempts.1 = Some(Instant::now());
                log::warn!("[eversilver:pairing] Max attempts reached, lockout activated");
            }
        }
