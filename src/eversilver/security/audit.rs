...
use crate::eversilver::config::AuditConfig;
use anyhow::Result;
...
impl AuditLogger {
    ...
    pub fn new(config: AuditConfig, eversilver_dir: PathBuf) -> Result<Self> {
        ...
        log::info!(
            "[eversilver:audit] Logger initialized: enabled={}, path={}",
            config.enabled,
            log_path.display()
        );
        Ok(Self {
    ...
    pub fn log(&self, event: &AuditEvent) -> Result<()> {
        ...
        log::debug!(
            "[eversilver:audit] Logging event: type={:?}, id={}",
            event.event_type,
            event.event_id
        );
    ...
    fn rotate(&self) -> Result<()> {
        log::info!(
            "[eversilver:audit] Rotating audit log: {}",
