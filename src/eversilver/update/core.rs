//! Core self-update logic: check GitHub Releases for a newer `eversilver-core` binary
...
use crate::eversilver::update::types::{GitHubAsset, GitHubRelease, UpdateApplyResult, UpdateInfo};
use crate::eversilver::util::utf8_safe_prefix_at_byte_boundary;

/// GitHub owner/repo for the core binary releases.
const GITHUB_OWNER: &str = "eversilver";
const GITHUB_REPO: &str = "eversilver";
...
///
/// Convention: assets are named `eversilver-core-{triple}` (or `.exe` on Windows).
fn find_platform_asset(assets: &[GitHubAsset]) -> Option<&GitHubAsset> {
    let triple = platform_triple();
    let expected_name = format!("eversilver-core-{triple}");
...
/// Check GitHub Releases for a newer version of eversilver-core.
pub async fn check_available() -> Result<UpdateInfo, String> {
    ...
    let client = reqwest::Client::builder()
        .user_agent("eversilver-core-updater")
