...
pub fn format_recent_reflections_for_prompt(
    reflections: &[crate::eversilver::subconscious::reflection::Reflection],
) -> String {
    crate::eversilver::subconscious::situation_report::reflections::build_section(reflections)
}
...
fn resolve_prompts_dir(workspace_dir: &Path) -> Option<std::path::PathBuf> {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        return crate::eversilver::dev_paths::repo_ai_prompts_dir(&cwd);
    }
...
mod tests {
    use super::*;
    use crate::eversilver::subconscious::types::{TaskRecurrence, TaskSource};
