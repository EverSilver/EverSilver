...
async fn execute_with_local_model(
    ...
    let messages = vec![
        crate::eversilver::local_ai::ops::LocalAiChatMessage {
    ...
    let outcome = crate::eversilver::local_ai::ops::local_ai_chat(&config, messages, None)
        .await
...
async fn execute_with_agent_full(
    config: &mut crate::eversilver::config::Config,
    task: &SubconsciousTask,
    situation_report: &str,
    identity_context: &str,
) -> Result<String, String> {
...
async fn agent_chat_with_retry(
    ...
    loop {
        let result =
            crate::eversilver::local_ai::ops::agent_chat(config, prompt, None, Some(0.3)).await;
...
"#
        ...
        let _: crate::eversilver::config::Config =
    ...
    async fn execute_task_routes_to_cloud_when_local_disabled() {
        let _env_lock = crate::eversilver::config::TEST_ENV_LOCK
            .lock()
