...
use crate::eversilver::config::schema::NodeConfig;
...
impl NodeBootstrap {
    ...
    ///    if they set one).
    /// 2. OS user cache (`dirs::cache_dir()/eversilver/node-runtime`) — the
    ...
    fn cache_root(&self) -> PathBuf {
        ...
        if let Some(user_cache) = dirs::cache_dir() {
            return user_cache.join("eversilver").join("node-runtime");
        }
        tracing::warn!(
