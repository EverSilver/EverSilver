...
//! the title with `Eversilver:` makes it trivial for the user to tell
...
/// naturally (`Eversilver: New message from …`).
pub const EVERSILVER_TITLE_PREFIX: &str = "Eversilver: ";
...
///
/// Layout: `Eversilver: <Provider> — <raw title>` when both pieces are
/// present, collapsing to `Eversilver: <Provider>` when the raw title is
...
pub fn format_title(provider_label: &str, raw_title: &str) -> String {
    ...
    if raw.is_empty() {
        ...
        format!("{EVERSILVER_TITLE_PREFIX}{provider_label} — {raw}")
    }
}
...
mod tests {
    ...
    fn prefix_empty_title_falls_back_to_provider_only() {
        assert_eq!(format_title("Slack", ""), "Eversilver: Slack");
        assert_eq!(format_title("Slack", "   "), "Eversilver: Slack");
    }
    ...
    fn prefix_with_title_joins_with_em_dash() {
        assert_eq!(
            ...
            "Eversilver: Gmail — New mail from Alice"
        );
    }
