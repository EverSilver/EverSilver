...
use crate::eversilver::config::{Config, LearningConfig, ReflectionSource};
use crate::eversilver::learning::candidate::{self, CueFamily, EvidenceRef, FacetClass};
use crate::eversilver::memory::{Memory, MemoryCategory};
...
pub struct ReflectionHook {
    ...
    provider: Option<Arc<dyn crate::eversilver::providers::Provider>>,
...
impl ReflectionHook {
    ...
    async fn run_reflection(&self, prompt: &str) -> anyhow::Result<String> {
        match self.config.reflection_source {
            ReflectionSource::Local => {
                ...
                let service = crate::eversilver::local_ai::global(&self.full_config);
                service
    ...
    fn emit_candidates_from_reflection(output: &ReflectionOutput, episodic_id: i64) {
        ...
        for (n, pref) in output.user_preferences.iter().enumerate() {
            ...
            buf.push(crate::eversilver::learning::candidate::LearningCandidate {
...
impl PostTurnHook for ReflectionHook {
    ...
    async fn on_turn_complete(&self, ctx: &TurnContext) -> anyhow::Result<()> {
        ...
        if self.config.enabled {
            ...
            let episodic_id = crate::eversilver::learning::candidate::global().len() as i64;
