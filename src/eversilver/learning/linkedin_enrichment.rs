...
use crate::eversilver::integrations::{build_client, IntegrationClient};
use regex::Regex;
use serde_json::json;
...
pub async fn summarise_profile_with_llm(config: &Config, raw_md: &str) -> anyhow::Result<String> {
    use crate::eversilver::providers::ops::{
    ...
    // `EversilverBackendProvider::resolve_bearer` looks in `~/.eversilver`
...
fn build_memory_client() -> anyhow::Result<crate::eversilver::memory::store::MemoryClient> {
    crate::eversilver::memory::store::MemoryClient::new_local()
...
async fn persist_linkedin_profile(
    memory: &crate::eversilver::memory::store::MemoryClient,
    url: &str,
    data: &serde_json::Value,
) -> anyhow::Result<()> {
    let content = format!(
