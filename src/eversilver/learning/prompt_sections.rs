...
use crate::eversilver::context::prompt::{PromptContext, PromptSection};
use anyhow::Result;
...
impl LearnedContextSection {
    pub fn new(_memory: std::sync::Arc<dyn crate::eversilver::memory::Memory>) -> Self {
...
pub fn load_learned_from_cache(
    cache: &crate::eversilver::learning::cache::FacetCache,
) -> Vec<String> {
    ...
    use crate::eversilver::memory::store::profile::ProfileFacet;
    ...
    'outer: for indices in by_class.values() {
        for &idx in indices {
            ...
            let pinned =
                if f.user_state == crate::eversilver::memory::store::profile::UserState::Pinned {
                    " *(pinned)*"
                } else {
                    ""
                };
...
mod tests {
    use super::*;
    use crate::eversilver::context::prompt::LearnedContextData;
    ...
    fn load_learned_from_cache_formats_active_facets() {
        ...
        use crate::eversilver::memory::store::profile::{
    ...
    fn load_learned_from_cache_empty_when_no_active_facets() {
        use crate::eversilver::learning::cache::FacetCache;
        use crate::eversilver::memory::store::profile::PROFILE_INIT_SQL;
        use parking_lot::Mutex;
        use rusqlite::Connection;
