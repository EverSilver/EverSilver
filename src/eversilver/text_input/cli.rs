//! `eversilver text-input` — standalone CLI for text input intelligence.
//!
...
//!
//! Usage:
//!   eversilver text-input run       [--port <u16>] [-v]
//!   eversilver text-input read      [-v] [--bounds]
//!   eversilver text-input insert    <text> [-v]
//!   eversilver text-input ghost     <text> [--ttl <ms>] [-v]
//!   eversilver text-input dismiss   [-v]

use anyhow::Result;

/// Entry point for `eversilver text-input <subcommand>`.
pub(crate) fn run_text_input_command(args: &[String]) -> Result<()> {
    ...
    match args[0].as_str() {
        ...
        other => Err(anyhow::anyhow!(
            "unknown text-input subcommand '{other}'. Run `eversilver text-input --help`."
        )),
    }
}
...
/// `eversilver text-input run` — start a minimal JSON-RPC server.
fn run_server(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) {
        println!("Usage: eversilver text-input run [--port <u16>] [-v]");
        println!();
...
/// `eversilver text-input read` — one-shot read of the focused field.
fn run_read(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: eversilver text-input read [--bounds] [-v]");
        println!();
    ...
    rt.block_on(async {
        let params = crate::eversilver::text_input::ReadFieldParams {
            include_bounds: Some(opts.include_bounds),
        };
        let outcome = crate::eversilver::text_input::rpc::read_field(params)
            .await
...
/// `eversilver text-input insert <text>` — insert text into the focused field.
fn run_insert(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: eversilver text-input insert <text> [-v]");
        println!();
    ...
    rt.block_on(async {
        let params = crate::eversilver::text_input::InsertTextParams {
            text,
            validate_focus: None,
            expected_app: None,
            expected_role: None,
        };
        let outcome = crate::eversilver::text_input::rpc::insert_text(params)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
/// `eversilver text-input ghost <text>` — show ghost text overlay.
fn run_ghost(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: eversilver text-input ghost <text> [--ttl <ms>] [-v]");
        println!();
    ...
    rt.block_on(async {
        let params = crate::eversilver::text_input::ShowGhostTextParams {
            text,
            ttl_ms: Some(opts.ttl_ms),
            bounds: None,
        };
        let outcome = crate::eversilver::text_input::rpc::show_ghost(params)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
/// `eversilver text-input dismiss` — dismiss the ghost text overlay.
fn run_dismiss(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: eversilver text-input dismiss [-v]");
        println!();
    ...
    rt.block_on(async {
        let outcome = crate::eversilver::text_input::rpc::dismiss_ghost()
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
fn print_help() {
    println!("eversilver text-input — text input intelligence\n");
    println!("Usage:");
    println!("  eversilver text-input run       [--port <u16>] [-v]");
    println!("  eversilver text-input read      [--bounds] [-v]");
    println!("  eversilver text-input insert    <text> [-v]");
    println!("  eversilver text-input ghost     <text> [--ttl <ms>] [-v]");
    println!("  eversilver text-input dismiss   [-v]");
    println!();
    println!("Subcommands:");
