...
//! [`crate::eversilver::local_ai::voice_install_common`] status table so
...
/// [`crate::eversilver::voice::factory::DEFAULT_WHISPER_MODEL`].
...
pub fn status(config: &Config) -> VoiceInstallStatus {
    ...
    if matches!(snapshot.state, VoiceInstallState::Missing) {
        let configured = crate::eversilver::local_ai::model_ids::effective_stt_model_id(config);
...
mod tests {
    use super::*;
    use crate::eversilver::local_ai::voice_install_common::reset_status;
    ...
    /// Serialise tests that write into the shared `~/.eversilver/bin/whisper/`
