...
use crate::eversilver::config::rpc as config_rpc;
...
fn handle_agent_chat(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::eversilver::local_ai::rpc::agent_chat(
                &mut config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_agent_chat_simple(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::eversilver::local_ai::rpc::agent_chat_simple(
                &config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_local_ai_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::eversilver::local_ai::rpc::local_ai_status(&config).await?)
    })
}
...
fn handle_local_ai_shutdown_owned(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let mut config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::eversilver::local_ai::rpc::local_ai_shutdown_owned(&mut config).await?)
    })
}
...
fn handle_local_ai_download(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_download(&config, p.force.unwrap_or(false))
                .await?,
        )
    })
}
...
fn handle_local_ai_download_all_assets(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_download_all_assets(
                &config,
...
fn handle_local_ai_summarize(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_summarize(&config, &p.text, p.max_tokens)
                .await?,
        )
    })
}
...
fn handle_local_ai_prompt(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_prompt(
                &config,
                &p.prompt,
                p.max_tokens,
                p.no_think,
...
fn handle_local_ai_vision_prompt(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_vision_prompt(
                &config,
                &p.prompt,
                &p.image_refs,
                p.max_tokens,
...
fn handle_local_ai_embed(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::eversilver::local_ai::rpc::local_ai_embed(&config, &p.inputs).await?)
    })
}
...
fn handle_local_ai_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_transcribe(&config, p.audio_path.trim())
                .await?,
        )
    })
}
...
fn handle_local_ai_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
            )
...
fn handle_local_ai_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_tts(
                &config,
                &p.text,
...
fn handle_local_ai_assets_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::eversilver::local_ai::rpc::local_ai_assets_status(&config).await?)
    })
}
...
fn handle_local_ai_downloads_progress(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::eversilver::local_ai::rpc::local_ai_downloads_progress(&config).await?)
    })
}
...
fn handle_local_ai_download_asset(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_download_asset(&config, p.capability.trim())
                .await?,
        )
    })
}
...
fn handle_local_ai_device_profile(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let profile = crate::eversilver::local_ai::device::detect_device_profile();
...
fn handle_local_ai_presets(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        let device = crate::eversilver::local_ai::device::detect_device_profile();
        let recommended = crate::eversilver::local_ai::presets::recommend_tier(&device);
        let current =
            crate::eversilver::local_ai::presets::current_tier_from_config(&config.local_ai);
        let selected_tier = config.local_ai.selected_tier.as_ref().and_then(|value| {
            let normalized = value.trim().to_ascii_lowercase();
            crate::eversilver::local_ai::presets::ModelTier::from_str_opt(&normalized)
                .map(|tier| tier.as_str().to_string())
                .or_else(|| (!normalized.is_empty()).then_some(normalized))
        });
        let presets = crate::eversilver::local_ai::presets::mvp_presets();
        tracing::debug!(
            ?recommended,
            ?current,
        ...
        let recommend_disabled =
            crate::eversilver::local_ai::presets::should_default_to_cloud_fallback(&device);
...
fn handle_local_ai_apply_preset(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let tier = crate::eversilver::local_ai::presets::ModelTier::from_str_opt(&tier_str)
            .ok_or_else(|| {
                format!(
        ...
        if tier == crate::eversilver::local_ai::presets::ModelTier::Custom {
        ...
        crate::eversilver::local_ai::presets::apply_preset_to_config(&mut config.local_ai, tier);
        config
            .save()
            .await
            .map_err(|e| format!("save config: {e}"))?;
        ...
        Ok(serde_json::json!({
            ...
            "vision_mode": crate::eversilver::local_ai::presets::vision_mode_for_config(&config.local_ai),
...
fn handle_local_ai_diagnostics(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let service = crate::eversilver::local_ai::global(&config);
...
fn handle_local_ai_should_react(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_should_react(
                &config,
                &p.message,
                &p.channel_type,
            )
...
fn handle_local_ai_analyze_sentiment(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::sentiment::local_ai_analyze_sentiment(&config, &p.message)
                .await?,
        )
    })
}
...
fn handle_local_ai_should_send_gif(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::gif_decision::local_ai_should_send_gif(
                &config,
                &p.message,
                &p.channel_type,
            )
...
fn handle_local_ai_tenor_search(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::eversilver::local_ai::gif_decision::tenor_search(&config, &p.query, p.limit)
                .await?,
        )
    })
}
...
fn handle_local_ai_chat(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        let messages: Vec<crate::eversilver::local_ai::rpc::LocalAiChatMessage> = p
            .messages
            .into_iter()
            .map(|m| crate::eversilver::local_ai::rpc::LocalAiChatMessage {
                role: m.role,
                content: m.content,
            })
            .collect();
        to_json(
            crate::eversilver::local_ai::rpc::local_ai_chat(&config, messages, p.max_tokens).await?,
        )
    })
}
...
fn handle_local_ai_install_whisper(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let slot = match crate::eversilver::local_ai::voice_install_common::try_acquire_install_slot(
            crate::eversilver::local_ai::voice_install_common::ENGINE_WHISPER,
        ) {
            Some(slot) => slot,
            None => {
                tracing::debug!(
                ...
                let current = crate::eversilver::local_ai::voice_install_common::read_status(
                    crate::eversilver::local_ai::voice_install_common::ENGINE_WHISPER,
                );
        ...
        crate::eversilver::local_ai::voice_install_common::write_status(
            crate::eversilver::local_ai::voice_install_common::VoiceInstallStatus {
                engine: crate::eversilver::local_ai::voice_install_common::ENGINE_WHISPER
                    .to_string(),
                state:
                    crate::eversilver::local_ai::voice_install_common::VoiceInstallState::Installing,
                progress: Some(0),
        ...
        tokio::spawn(async move {
            let _slot = slot;
            if let Err(e) = crate::eversilver::local_ai::install_whisper::install_whisper(
        ...
        let status = crate::eversilver::local_ai::voice_install_common::read_status(
            crate::eversilver::local_ai::voice_install_common::ENGINE_WHISPER,
        );
...
fn handle_local_ai_install_piper(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let slot = match crate::eversilver::local_ai::voice_install_common::try_acquire_install_slot(
            crate::eversilver::local_ai::voice_install_common::ENGINE_PIPER,
        ) {
            Some(slot) => slot,
            None => {
                tracing::debug!(
        ...
        crate::eversilver::local_ai::voice_install_common::write_status(
            crate::eversilver::local_ai::voice_install_common::VoiceInstallStatus {
                engine: crate::eversilver::local_ai::voice_install_common::ENGINE_PIPER.to_string(),
                state:
                    crate::eversilver::local_ai::voice_install_common::VoiceInstallState::Installing,
                progress: Some(0),
                downloaded_bytes: None,
        ...
        tokio::spawn(async move {
            let _slot = slot;
            if let Err(e) =
                crate::eversilver::local_ai::install_piper::install_piper(&config, voice_id, force)
                    .await
            {
...
fn handle_local_ai_whisper_install_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let status = crate::eversilver::local_ai::install_whisper::status(&config);
        serde_json::to_value(status).map_err(|e| format!("serialize whisper status: {e}"))
    })
}
...
fn handle_local_ai_piper_install_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let status = crate::eversilver::local_ai::install_piper::status(&config);
        serde_json::to_value(status).map_err(|e| format!("serialize piper status: {e}"))
    })
}
