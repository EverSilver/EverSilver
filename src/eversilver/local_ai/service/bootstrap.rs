...
use crate::eversilver::local_ai::device::DeviceProfile;
...
use crate::eversilver::local_ai::types::LocalAiStatus;
    ...
    pub async fn bootstrap(&self, config: &Config) {
        ...
        if effective_config.local_ai.whisper_in_process {
            if let Ok(model_path) =
                crate::eversilver::local_ai::paths::resolve_stt_model_path(&effective_config)
            {
...
fn config_with_recommended_tier_if_unselected(config: &Config, device: &DeviceProfile) -> Config {
    ...
    if !config.local_ai.opt_in_confirmed {
        tracing::debug!(
            ...
            min_required_gb = crate::eversilver::local_ai::presets::MIN_RAM_GB_FOR_LOCAL_AI,
            ?current_tier,
...
fn format_degraded_warning(err: &str, config: &Config) -> String {
    let current = crate::eversilver::local_ai::presets::current_tier_from_config(&config.local_ai);
    match current {
        crate::eversilver::local_ai::presets::ModelTier::Ram16PlusGb => {
            format!(
        ...
        crate::eversilver::local_ai::presets::ModelTier::Ram8To16Gb => {
            format!(
        ...
        crate::eversilver::local_ai::presets::ModelTier::Ram4To8Gb => format!(
...
mod tests {
    ...
    fn bootstrap_honors_opt_in_on_low_ram_device() {
        ...
        crate::eversilver::local_ai::presets::apply_preset_to_config(
            &mut config.local_ai,
            crate::eversilver::local_ai::presets::ModelTier::Ram2To4Gb,
        );
        let device = test_device(4);
