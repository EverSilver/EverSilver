...
async fn lm_studio_chat_with_history_returns_response() {
    ...
    let reply = service
        .chat_with_history(
            &config,
            vec![
                crate::eversilver::local_ai::ollama_api::OllamaChatMessage {
...
async fn inline_complete_interactive_does_not_block_on_held_permit() {
    ...
    let _held = crate::eversilver::scheduler_gate::gate::try_acquire_llm_permit()
...
async fn gated_inline_complete_blocks_on_held_permit() {
    ...
    let held = crate::eversilver::scheduler_gate::gate::try_acquire_llm_permit()
