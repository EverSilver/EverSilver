//! Spawn marker for eversilver-owned `ollama serve` processes.
//!
...
//! recording the PID, the binary we launched, and the eversilver process
//! that owned it. On graceful shutdown the marker is cleared. If eversilver
...
use crate::eversilver::local_ai::paths::ollama_spawn_marker_path;

/// On-disk record of an eversilver-spawned `ollama serve` process.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct OllamaSpawnMarker {
    pub pid: u32,
    pub started_at_unix: u64,
    pub binary_path: String,
    pub eversilver_pid: u32,
}
...
impl OllamaSpawnMarker {
    pub(crate) fn new(pid: u32, binary_path: &Path) -> Self {
        ...
        Self {
            ...
            eversilver_pid: std::process::id(),
        }
    }
}

// ---- Path-keyed helpers (testable without touching ~/.eversilver/) -------
...
mod tests {
    ...
    fn new_marker_captures_current_process_id() {
        ...
        assert_eq!(m.eversilver_pid, std::process::id());
