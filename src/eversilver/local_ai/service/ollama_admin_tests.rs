...
use crate::eversilver::local_ai::service::LocalAiService;
...
async fn has_model_detects_exact_and_prefixed_tag() {
    let _guard = crate::eversilver::local_ai::local_ai_test_guard();
...
async fn diagnostics_ok_when_expected_models_are_present() {
    ...
    let config = Config::default();
    let chat = crate::eversilver::local_ai::model_ids::effective_chat_model_id(&config);
    let embedding = crate::eversilver::local_ai::model_ids::effective_embedding_model_id(&config);
...
// daemons eversilver spawned itself, and that the kill path actually
...
// user started outside eversilver — the issue #1622 friendly-fire bug).

#[tokio::test]
async fn kill_ollama_server_with_no_owned_child_is_noop() {
    let _guard = crate::eversilver::local_ai::LOCAL_AI_TEST_MUTEX
        .lock()
...
async fn kill_ollama_server_kills_owned_child() {
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    assert!(
        crate::eversilver::local_ai::service::spawn_marker::pid_is_alive(pid),
    ...
    for _ in 0..40 {
        if !crate::eversilver::local_ai::service::spawn_marker::pid_is_alive(pid) {
            still_alive = false;
            break;
        }
...
async fn shutdown_owned_ollama_clears_marker_and_kills_child() {
    ...
    // touch the real `~/.eversilver/`. Per `paths::shared_root_dir`, when
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    // dir, we skip the marker assertion to avoid touching `~/.eversilver/`.
    let marker_path = crate::eversilver::local_ai::paths::ollama_spawn_marker_path(&config);
    let marker_writable = marker_path.starts_with(tmp.path());
    if marker_writable {
        crate::eversilver::local_ai::service::spawn_marker::write_marker_at(
            &marker_path,
            &crate::eversilver::local_ai::service::spawn_marker::OllamaSpawnMarker::new(
                pid,
