...
use crate::eversilver::local_ai::install::{find_system_ollama_binary, run_ollama_install_script};
use crate::eversilver::local_ai::lm_studio_api::lm_studio_base_url;
use crate::eversilver::local_ai::model_ids;
use crate::eversilver::local_ai::ollama_api::{
    ollama_base_url, OllamaModelTag, OllamaPullEvent, OllamaPullProgress, OllamaPullRequest,
    OllamaTagsResponse,
};
use crate::eversilver::local_ai::paths::{find_workspace_ollama_binary, workspace_ollama_binary};
use crate::eversilver::local_ai::presets::{self, VisionMode};
use crate::eversilver::local_ai::process_util::apply_no_window;
use crate::eversilver::local_ai::provider::{provider_from_config, LocalAiProvider};
...
impl LocalAiService {
    pub(in crate::eversilver::local_ai::service) async fn ensure_ollama_server(
        &self,
        config: &Config,
    ) -> Result<(), String> {
        // If eversilver crashed last session and left a daemon running, the
        ...
        if self.ollama_healthy().await {
            ...
            if self.ollama_healthy().await {
                ...
                return Err("An external Ollama daemon on :11434 has a broken runner. \
                     Restart it manually (or stop it so eversilver can take over)."
                    .to_string());
            }
        }
    ...
    pub(in crate::eversilver::local_ai::service) async fn ensure_ollama_server_fresh(
        &self,
        config: &Config,
    ) -> Result<(), String> {
    ...
    /// Check if a healthy daemon on `:11434` is actually eversilver's own
    ...
    async fn reclaim_orphan_if_ours(&self, config: &Config) {
        ...
        log::info!(
            "[local_ai] reclaiming eversilver-owned ollama orphan from prior session \
             (pid={}, binary={})",
            marker.pid,
            marker.binary_path
        );
    ...
    async fn start_and_wait_for_server(
        ...
        if self.ollama_healthy().await {
            ...
            // so `owned_ollama` stays `None` and the daemon survives eversilver
        ...
        for _ in 0..20 {
            if self.ollama_healthy().await {
                ...
                // write the spawn marker so a crashed eversilver can reclaim
    ...
    async fn download_and_install_ollama(&self, config: &Config) -> Result<(), String> {
        let install_dir = crate::eversilver::local_ai::paths::workspace_ollama_dir(config);
        ...
        // It outlives Eversilver crashing, the user closing the app, or
        ...
        if crate::eversilver::local_ai::install::is_ollama_installer_running() {
            log::info!(
            ...
            while crate::eversilver::local_ai::install::is_ollama_installer_running() {
    ...
    pub(in crate::eversilver::local_ai::service) fn ollama_binary_present(
        &self,
        config: &Config,
    ) -> bool {
    ...
    pub(in crate::eversilver::local_ai::service) async fn ensure_models_available(
        &self,
        config: &Config,
    ) -> Result<(), String> {
    ...
    pub(in crate::eversilver::local_ai::service) async fn ensure_ollama_model_available(
        &self,
        model_id: &str,
        label: &str,
    ) -> Result<(), String> {
    ...
    fn resolve_binary_path(&self, config: &Config) -> Option<String> {
        ...
        crate::eversilver::local_ai::install::find_system_ollama_binary()
    ...
    /// Kill the `ollama serve` daemon eversilver itself spawned, if any.
    ///
    /// **No-op when eversilver never spawned a daemon** (i.e. it adopted an
    ...
    async fn kill_ollama_server(&self) {
        ...
        let Some(mut child) = maybe_child else {
            log::debug!(
                "[local_ai] kill_ollama_server: no eversilver-owned daemon; \
        ...
        match child.kill().await {
            Ok(()) => {
                log::info!("[local_ai] killed eversilver-owned ollama serve (pid={pid})");
    ...
    ///
    /// Kills the eversilver-owned `ollama serve` (if any) and clears the
    ...
    pub(in crate::eversilver::local_ai::service) async fn has_model(
        &self,
        model: &str,
    ) -> Result<bool, String> {
...
/// `tokio::process::Child` handle (the spawning eversilver crashed) but
