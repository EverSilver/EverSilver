...
use crate::eversilver::local_ai::parse::sanitize_inline_completion;
...
impl LocalAiService {
    ...
    pub(crate) async fn chat_with_history(
        &self,
        config: &Config,
        messages: Vec<crate::eversilver::local_ai::ollama_api::OllamaChatMessage>,
        max_tokens: Option<u32>,
    ) -> Result<String, String> {
        if !config.local_ai.runtime_enabled {
            return Err("local ai is disabled".to_string());
        ...
        let _gate_permit = crate::eversilver::scheduler_gate::wait_for_capacity().await;
        ...
        if provider_from_config(config) == LocalAiProvider::LmStudio {
            ...
            let lm_messages = messages
                .into_iter()
                .map(
                    |message| crate::eversilver::local_ai::lm_studio_api::LmStudioChatMessage {
                        role: message.role,
        ...
        tracing::debug!(
            ...
            model = %crate::eversilver::local_ai::model_ids::effective_chat_model_id(config),
            "[local_ai:chat] sending to ollama /api/chat"
        );

        let started = std::time::Instant::now();

        let body = crate::eversilver::local_ai::ollama_api::OllamaChatRequest {
            model: crate::eversilver::local_ai::model_ids::effective_chat_model_id(config),
            messages,
            stream: false,
            options: Some(
                crate::eversilver::local_ai::ollama_api::OllamaGenerateOptions {
        ...
        let payload: crate::eversilver::local_ai::ollama_api::OllamaChatResponse = response
            .json()
            .await
    ...
    async fn inference_with_temperature_internal(
        ...
        let _gate_permit = if gated {
            crate::eversilver::scheduler_gate::wait_for_capacity().await
        } else {
            None
        };
        ...
        if provider_from_config(config) == LocalAiProvider::LmStudio {
            let messages = vec![
                crate::eversilver::local_ai::lm_studio_api::LmStudioChatMessage {
        ...
        let payload: crate::eversilver::local_ai::ollama_api::OllamaGenerateResponse = response
            .json()
            .await
