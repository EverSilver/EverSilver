...
///
/// Default callers see the shared `~/.eversilver/` root, which avoids
...
/// Spawn marker file recording the PID of any `ollama serve` eversilver
/// itself spawned. Read on next launch to recognise our own orphan when
/// eversilver crashed before its graceful-shutdown hook ran. Lives under
...
pub(crate) fn resolve_whisper_binary() -> Option<PathBuf> {
    ...
    if let Ok(shared) = crate::eversilver::config::default_root_eversilver_dir() {
...
mod tests {
    ...
    fn resolve_tts_voice_path_appends_onnx_for_voice_ids() {
        ...
        // root (`~/.eversilver/`) — not the temp config. If a sibling
