...
/// Returns the root eversilver directory (`~/.eversilver`), independent of any
...
fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir
        .parent()
        .map(|parent| parent.join(".eversilver"));
...
async fn resolve_config_dirs_ignoring_env(
    ...
    // 2. Active user — scopes the entire eversilver dir to a per-user directory
...
fn decrypt_optional_secret(
    store: &crate::eversilver::security::SecretStore,
    value: &mut Option<String>,
    field_name: &str,
) -> Result<()> {
    if let Some(raw) = value.clone() {
        if crate::eversilver::security::SecretStore::is_encrypted(&raw) {
            *value = Some(
                store
                    .decrypt(&raw)
...
/// use the default (unauthenticated) eversilver directory.
...
/// Returns the user-scoped eversilver directory for the given user id:
...
/// Stable id used to scope the eversilver directory before any user has
...
/// so nothing is ever written directly at the root `.eversilver` path.
///
...
fn migrate_cloud_provider_slugs(config: &mut Config) {
    ...
    // Legacy bare strings are: "cloud", "eversilver", "openai", "anthropic",
    ...
    let rewrite = |field: &mut Option<String>| {
        ...
        // Already in new grammar (contains ':') or is the eversilver sentinel.
        if raw.contains(':') || raw == "eversilver" {
            return;
        }
        match raw.as_str() {
            "cloud" => {
                // "cloud" sentinel: look for the primary or first non-eversilver entry.
                // If none found, leave as "eversilver".
                ...
                if let Some(s) = slug {
                    ...
                    *field = Some(format!("{s}:"));
                    ...
                    tracing::debug!(
                        "[config][migrate] routing 'cloud' with no non-eversilver provider → 'eversilver'"
                    );
                    *field = Some("eversilver".to_string());
                }
            }
            other => {
...
impl Config {
    ...
    async fn load_or_init_with_env_lookup(
        ...
        if config_path.exists() {
            ...
            crate::eversilver::migrations::run_pending(&mut config).await;
            Ok(config)
        } else {
            ...
            let mut config = Config {
                config_path: config_path.clone(),
                workspace_dir,
                schema_version: crate::eversilver::migrations::CURRENT_SCHEMA_VERSION,
                ..Default::default()
            };
            config.save().await?;
    ...
    pub(crate) fn apply_env_overlay_with<E: EnvLookup + ?Sized>(&mut self, env: &E) {
        ...
        if let Some(tier_str) = env.get("EVERSILVER_LOCAL_AI_TIER") {
            ...
            if !tier_str.is_empty() {
                if let Some(tier) =
                    crate::eversilver::local_ai::presets::ModelTier::from_str_opt(&tier_str)
                {
                    if tier == crate::eversilver::local_ai::presets::ModelTier::Custom {
                        ...
                        crate::eversilver::local_ai::presets::apply_preset_to_config(
                            &mut self.local_ai,
                            tier,
                        );
            ...
            match normalized.as_str() {
                "local" => {
                    self.learning.reflection_source =
                        crate::eversilver::config::ReflectionSource::Local
                }
                "cloud" => {
                    self.learning.reflection_source =
        ...
        if let Some(raw) = env.get("EVERSILVER_MEMORY_TREE_LLM_BACKEND") {
            ...
            if !trimmed.is_empty() {
                match crate::eversilver::config::LlmBackend::parse(trimmed) {
                    Ok(b) => {
                        log::debug!(
            ...
            match normalized.as_str() {
                "toggle" => {
                    self.dictation.activation_mode =
                        crate::eversilver::config::DictationActivationMode::Toggle
                }
                "push" => {
                    self.dictation.activation_mode =
                        crate::eversilver::config::DictationActivationMode::Push
                }
                _ => {
                    tracing::warn!(
                        mode = %mode,
        ...
        let context_default = crate::eversilver::context::DEFAULT_TOOL_RESULT_BUDGET_BYTES;
