...
fn user_eversilver_dir_builds_correct_path() {
    let root = PathBuf::from("/home/test/.eversilver");
    let dir = user_eversilver_dir(&root, "user-123");
    assert_eq!(dir, PathBuf::from("/home/test/.eversilver/users/user-123"));
}
...
fn pre_login_user_dir_is_under_users_tree() {
    ...
    assert_eq!(
        dir,
        PathBuf::from("/home/test/.eversilver/users").join(PRE_LOGIN_USER_ID)
    );
}
...
fn resolve_config_dir_for_workspace_returns_parent_and_workspace() {
    let ws = PathBuf::from("/home/test/.eversilver/workspace");
    let (config_dir, workspace_dir) = resolve_config_dir_for_workspace(&ws);
    // Config dir is the parent of workspace.
    assert!(
        config_dir.ends_with(".eversilver") || config_dir == PathBuf::from("/home/test/.eversilver")
    );
...
fn env_overlay_context_tool_result_budget_env_suppresses_legacy_migration() {
    ...
    // the default.
    let default_budget = crate::eversilver::context::DEFAULT_TOOL_RESULT_BUDGET_BYTES;
    let mut cfg = Config::default();
...
fn env_overlay_auto_update_restart_strategy_accepts_supported_values() {
    ...
    assert_eq!(
        ...
        crate::eversilver::config::UpdateRestartStrategy::Supervisor
    );
    ...
    assert_eq!(
        cfg.update.restart_strategy,
        crate::eversilver::config::UpdateRestartStrategy::SelfReplace
    );
}
...
fn apply_env_overrides_commits_side_effects_to_runtime_proxy() {
    use crate::eversilver::config::schema::proxy::{runtime_proxy_config, set_runtime_proxy_config};
...
async fn load_or_init_falls_back_to_defaults_when_backup_also_corrupted() {
    ...
    assert_eq!(
        config.default_model.as_deref(),
        Some(crate::eversilver::config::schema::DEFAULT_MODEL),
