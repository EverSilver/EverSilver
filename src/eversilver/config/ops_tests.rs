...
async fn reset_local_data_removes_current_dir_default_dir_and_marker() {
    ...
    let default_eversilver_dir = temp.path().join("default-eversilver");
    let current_eversilver_dir = temp.path().join("custom-eversilver");
    ...
    tokio::fs::create_dir_all(current_eversilver_dir.join("workspace"))
        .await
        .unwrap();
    tokio::fs::write(&marker, "config_dir = '/tmp/custom-eversilver'\n")
        .await
        .unwrap();
...
fn default_eversilver_dir_ends_in_dot_eversilver() {
    ...
    assert!(p.ends_with(".eversilver"));
}

#[test]
fn active_workspace_marker_path_is_under_default_dir() {
    let default_dir = std::path::Path::new("/tmp/eversilver-test");
...
fn reset_local_data_remove_error_explains_windows_file_locks() {
    ...
    let msg =
        reset_local_data_remove_error(std::path::Path::new("C:\\Users\\me\\.eversilver"), &err);
...
async fn apply_model_settings_replaces_model_routes_when_some_and_keeps_when_none() {
    ...
    use crate::eversilver::config::ModelRouteConfig;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn apply_memory_settings_updates_all_provided_fields() {
    ...
    assert_eq!(
        ...
        Some(crate::eversilver::config::schema::MemoryContextWindow::Extended)
    );
}
...
async fn apply_memory_settings_ignores_unknown_memory_window_label() {
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
    cfg.agent.memory_window = Some(crate::eversilver::config::schema::MemoryContextWindow::Balanced);
...
async fn apply_memory_settings_round_trips_all_window_labels() {
    use crate::eversilver::config::schema::MemoryContextWindow;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
