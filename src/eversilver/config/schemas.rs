...
pub fn schemas(function: &str) -> ControllerSchema {
    match function {
        ...
        "get_data_paths" => ControllerSchema {
            ...
            description:
                "Resolve the Eversilver data directories (current workspace, default ~/.eversilver, active workspace marker) that reset_local_data would remove. Read-only — performs no filesystem changes.",
            inputs: vec![],
    ...
    Box::pin(async move {
        ...
        let patch = config_rpc::ModelSettingsPatch {
            ...
            model_routes: update.model_routes.map(|routes| {
                routes
                    .into_iter()
                    .map(|r| crate::eversilver::config::ModelRouteConfig {
                        hint: r.hint,
                        model: r.model,
                    })
                    .collect()
                    ...
                    entries
                        ...
                        .map(|e| {
                                ...
                                .as_str()
                            {
                                ...
                                "eversilver_jwt" | "eversilverjwt" => AuthStyle::EversilverJwt,
                                "none" => AuthStyle::None,
                                other => {
                                    return Err(format!(
