...
/// Returns the default workspace directory fallback (~/.eversilver/workspace).
fn fallback_workspace_dir() -> PathBuf {
    crate::eversilver::config::default_root_eversilver_dir()
        .unwrap_or_else(|_| env_scoped_fallback_root_dir())
        .join("workspace")
}

/// Returns the default Eversilver configuration directory (~/.eversilver).
...
fn env_scoped_fallback_root_dir() -> PathBuf {
    ...
    PathBuf::from(format!(".eversilver{suffix}"))
}
...
pub struct ModelSettingsPatch {
    ...
    pub model_routes: Option<Vec<crate::eversilver::config::ModelRouteConfig>>,
    ...
    /// `auth-profiles.json` via [`crate::eversilver::credentials::AuthService`]).
    /// Pass `Some(vec![])` to clear all third-party cloud providers.
    pub cloud_providers:
        Option<Vec<crate::eversilver::config::schema::cloud_providers::CloudProviderCreds>>,
...
pub async fn apply_memory_settings(
    ...
    if let Some(window_label) = update.memory_window.as_deref() {
        if let Some(window) =
            crate::eversilver::config::schema::MemoryContextWindow::from_str_opt(window_label)
        {
...
pub async fn apply_local_ai_settings(
    ...
    if let Some(provider) = update.provider {
        config.local_ai.provider =
            crate::eversilver::local_ai::provider::normalize_provider(&provider);
    }
...
/// cron job via [`crate::eversilver::cron::seed::seed_proactive_agents`].
...
pub async fn set_onboarding_completed(value: bool) -> Result<RpcOutcome<bool>, String> {
    ...
    if value && !was_completed {
        ...
        tokio::task::spawn_blocking(move || {
            if let Err(e) = crate::eversilver::cron::seed::seed_proactive_agents(&seed_config) {
        ...
        match mode.as_str() {
            "tap" => {
                config.voice_server.activation_mode =
                    crate::eversilver::config::VoiceActivationMode::Tap;
            }
            "push" => {
                config.voice_server.activation_mode =
                    crate::eversilver::config::VoiceActivationMode::Push;
            }
            _ => {
                return Err(format!(
...
pub fn agent_server_status() -> RpcOutcome<serde_json::Value> {
    let running = crate::eversilver::service::mock::mock_agent_running().unwrap_or(true);
...
/// workspace dir, the default `~/.eversilver` dir (which can differ when
