//! Tauri-focused daemon configuration wrapper for eversilver.
...
pub struct DaemonConfig {
    /// Root data directory (defaults to Tauri's `app_data_dir/eversilver`).
...
impl DaemonConfig {
    ...
    pub fn from_app_data_dir(app_data_dir: &std::path::Path) -> Self {
        let data_dir = app_data_dir.join("eversilver");
        let workspace_dir = data_dir.join("workspace");
        log::info!(
            "[eversilver:config] Initialized config: data_dir={}, workspace_dir={}",
            data_dir.display(),
...
mod tests {
    ...
    fn daemon_config_from_app_data_dir() {
        let app_data = std::path::PathBuf::from("/tmp/test-eversilver");
        let config = DaemonConfig::from_app_data_dir(&app_data);

        assert_eq!(config.data_dir, app_data.join("eversilver"));
        assert_eq!(
            config.workspace_dir,
            app_data.join("eversilver").join("workspace")
        );
    }
}
