...
pub async fn start_login_gated_services(config: &Config) {
    ...
    crate::eversilver::voice::server::start_if_enabled(config).await;
    ...
    if !config.voice_server.auto_start {
        crate::eversilver::voice::dictation_listener::start_if_enabled(config).await;
    }

    // 4. Screen intelligence (capture + vision analysis)
    crate::eversilver::screen_intelligence::server::start_if_enabled(config).await;

    // 5. Autocomplete (text suggestions + Swift overlay helper)
    crate::eversilver::autocomplete::start_if_enabled(config).await;
...
pub async fn stop_login_gated_services(config: &Config) {
    ...
    if let Some(server) = crate::eversilver::screen_intelligence::server::try_global_server() {
        server.stop().await;
    ...
    crate::eversilver::voice::dictation_listener::stop();
...
pub async fn store_session(
    ...
    // Determine user_id so we can scope the eversilver directory to this user.
    ...
    let effective_config = if resolved_user_id.is_some() {
        match crate::eversilver::config::load_config_with_timeout().await {
            Ok(c) => c,
            Err(_) => config.clone(),
        }
    } else {
    ...
    if let Err(e) = crate::eversilver::subconscious::global::bootstrap_after_login().await {
...
pub async fn clear_session(config: &Config) -> Result<RpcOutcome<serde_json::Value>, String> {
    ...
    // default (unauthenticated) eversilver directory.
    if let Ok(root_dir) = default_root_eversilver_dir() {
        if let Err(e) = crate::eversilver::config::clear_active_user(&root_dir) {
    ...
    crate::eversilver::subconscious::global::reset_engine_for_user_switch().await;
...
///
/// Used by [`crate::eversilver::composio::client::create_composio_client`]
