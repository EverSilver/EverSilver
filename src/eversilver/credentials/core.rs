...
mod tests {
    use super::*;
    use crate::eversilver::credentials::profiles::{AuthProfile, AuthProfileKind};
    ...
    fn state_dir_from_config_uses_config_path_parent() {
        ...
        config.config_path = PathBuf::from("/tmp/eversilver-test/config.toml");
        assert_eq!(
            state_dir_from_config(&config),
            PathBuf::from("/tmp/eversilver-test")
        );
    }
