...
use crate::eversilver::autocomplete::AutocompleteStatus;
use crate::eversilver::config::rpc as config_rpc;
use crate::eversilver::config::Config;
use crate::eversilver::credentials::session_support::build_session_state;
use crate::eversilver::local_ai::LocalAiStatus;
use crate::eversilver::screen_intelligence::AccessibilityStatus;
use crate::eversilver::service::{ServiceState, ServiceStatus};
...
pub struct AppStateSnapshot {
    pub auth: crate::eversilver::credentials::responses::AuthStateResponse,
...
pub fn peek_cached_current_user_identity() -> Option<crate::eversilver::agent::prompts::UserIdentity>
{
    let cache = CURRENT_USER_CACHE.lock();
    ...
    let identity = crate::eversilver::agent::prompts::UserIdentity { id, name, email };
...
async fn build_runtime_snapshot(config: &Config) -> RuntimeSnapshot {
    let screen_intelligence = {
        let _ = crate::eversilver::screen_intelligence::global_engine()
            .apply_config(config.screen_intelligence.clone())
            .await;
        crate::eversilver::screen_intelligence::global_engine()
            .status()
            .await
    };

    let local_ai = match crate::eversilver::local_ai::rpc::local_ai_status(config).await {
        Ok(outcome) => outcome.value,
        Err(error) => {
            warn!("{LOG_PREFIX} local_ai status failed during snapshot: {error}");
            crate::eversilver::local_ai::LocalAiStatus::disabled(config)
        }
    };

    let autocomplete = crate::eversilver::autocomplete::global_engine()
        .status()
        .await;

    let service = match crate::eversilver::service::status(config) {
        Ok(status) => status,
        Err(error) => {
