...
vi.mock('../tauriCommands', () => ({
  eversilverLocalAiApplyPreset: vi.fn(),
  eversilverLocalAiDownloadAllAssets: vi.fn(),
  eversilverLocalAiPresets: vi.fn(),
}));
...
describe('localAiBootstrap', () => {
  ...
  it('applies the recommended preset before starting background downloads when no tier is selected', async () => {
    ...
    vi.mocked(tauriCommands.eversilverLocalAiPresets).mockResolvedValue({
      presets: [],
    ...
    vi.mocked(tauriCommands.eversilverLocalAiApplyPreset).mockResolvedValue({
    ...
    vi.mocked(tauriCommands.eversilverLocalAiDownloadAllAssets).mockResolvedValue({
    ...
    expect(tauriCommands.eversilverLocalAiPresets).toHaveBeenCalledOnce();
    expect(tauriCommands.eversilverLocalAiApplyPreset).toHaveBeenCalledWith('ram_2_4gb');
    expect(tauriCommands.eversilverLocalAiDownloadAllAssets).toHaveBeenCalledWith(false);
    expect(
      vi.mocked(tauriCommands.eversilverLocalAiApplyPreset).mock.invocationCallOrder[0]
    ).toBeLessThan(
      vi.mocked(tauriCommands.eversilverLocalAiDownloadAllAssets).mock.invocationCallOrder[0]
    );
  ...
  it('skips preset application when a tier is already selected', async () => {
    ...
    expect(tauriCommands.eversilverLocalAiApplyPreset).not.toHaveBeenCalled();
