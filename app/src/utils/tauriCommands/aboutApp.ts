 ...
 *
 * Thin wrapper around the `eversilver.about_app_*` JSON-RPC methods exposed by
 * the Rust core (`src/eversilver/about_app/schemas.rs`). The Privacy surface is
...
export async function listCapabilities(category?: CapabilityCategory): Promise<Capability[]> {
  const response = await callCoreRpc<CommandResponse<Capability[]> | Capability[]>({
    method: 'eversilver.about_app_list',
