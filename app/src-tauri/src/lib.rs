...
/// `EVERSILVER_CORE_TOKEN`, and stored in the handle — available immediately
...
fn overlay_parent_rpc_url() -> Option<String> {
    let url = std::env::var("EVERSILVER_CORE_RPC_URL").ok()?;
...
fn process_diagnostics_list_owned() -> Result<Vec<process_recovery::ProcessInfo>, String> {
    match process_recovery::enumerate_eversilver_processes() {
        Ok(processes) => {
            log::info!(
                "[startup-recovery] diagnostics listed {} owned Eversilver processes",
                processes.len()
            );
            Ok(processes)
        }
...
async fn reset_local_data(
    ...
    log::info!(
        ...
        paths.current_eversilver_dir.display(),
        paths.default_eversilver_dir.display(),
    ...
    // On Windows this is the load-bearing step for EVERSILVER-TAURI-AF.
...
struct ResolvedDataPaths {
    current_eversilver_dir: std::path::PathBuf,
    default_eversilver_dir: std::path::PathBuf,
...
async fn fetch_data_paths() -> Result<ResolvedDataPaths, String> {
    ...
    let current = inner
        .get("current_eversilver_dir")
        .and_then(|v| v.as_str())
        .ok_or_else(|| "config_get_data_paths missing current_eversilver_dir".to_string())?;
    let default = inner
        .get("default_eversilver_dir")
        .and_then(|v| v.as_str())
        .ok_or_else(|| "config_get_data_paths missing default_eversilver_dir".to_string())?;
    let marker = inner
    ...
    Ok(ResolvedDataPaths {
        current_eversilver_dir: std::path::PathBuf::from(current),
        default_eversilver_dir: std::path::PathBuf::from(default),
...
/// `cef_profile::default_root_eversilver_dir()` so the lookup honors
/// `EVERSILVER_WORKSPACE` overrides used in test harnesses. (#900)
#[tauri::command]
fn get_active_user_id() -> Result<Option<String>, String> {
    let dir = cef_profile::default_root_eversilver_dir()?;
...
/// missing (EVERSILVER-TAURI-AS). Walks `$PATH` entries, joins `name`, and
...
pub fn run() {
    ...
    // other startup work. Reads `EVERSILVER_TAURI_SENTRY_DSN` at runtime first,
    ...
    // `tauri::cef_entry_point`) and the `Eversilver core …` in-process core
    // path do NOT spin up a second client — those have their own reporting
    // surfaces.
    let _sentry_guard = sentry::init(sentry::ClientOptions {
        dsn: std::env::var("EVERSILVER_TAURI_SENTRY_DSN")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| option_env!("EVERSILVER_TAURI_SENTRY_DSN").map(|s| s.to_string()))
            .filter(|s| !s.is_empty())
        ...
        before_send: Some(std::sync::Arc::new(|mut event| {
            ...
            // don't want in Sentry (issue EVERSILVER-TAURI-V, 66+ events).
            ...
            // `eversilver_core::core::observability::is_max_iterations_event`
            // for the rationale). The shell links the core in-process so
            // any captured event for this deterministic agent-state
            // outcome is filtered here too (EVERSILVER-TAURI-99 / -98).
            ...
            // EVERSILVER-TAURI-25 / -1Q / -27 / -1G off Sentry.
    ...
    // Run with `EVERSILVER_TAURI_SENTRY_TEST=panic` to fire a panic, or
    // `=message` to send a captured-message event. No-op when unset.
    if let Ok(mode) = std::env::var("EVERSILVER_TAURI_SENTRY_TEST") {
        match mode.as_str() {
            "panic" => panic!("EVERSILVER_TAURI_SENTRY_TEST=panic — local Sentry smoke test"),
            "message" => {
                sentry::capture_message(
                    "EVERSILVER_TAURI_SENTRY_TEST=message — local Sentry smoke test",
                    sentry::Level::Error,
                );
                let _ = sentry::Hub::current().client().map(|c| c.flush(None));
            }
            other => log::warn!(
                "EVERSILVER_TAURI_SENTRY_TEST={other:?} — unknown mode (use 'panic' or 'message')"
            ),
        }
    }
    ...
    // ── Windows pre-CEF single-instance guard (Sentry EVERSILVER-TAURI-A) ──
    //
    ...
    let _cef_init_mutex_guard = {
        ...
        if unsafe { GetLastError() } == ERROR_ALREADY_EXISTS {
            ...
            log::info!(
                "[single-instance] pre-CEF mutex held by primary; secondary exiting (EVERSILVER-TAURI-A fix)"
            );
            std::process::exit(0);
        }
    ...
    // CEF cache-lock preflight (macOS only): if another Eversilver instance
    ...
    #[cfg(target_os = "macos")]
    process_recovery::reap_stale_eversilver_processes();
    ...
    let builder = {
        ...
        let mut args: Vec<CefCommandLineArg> = vec![
            ...
            // pumps mascot frames at 24 fps from the *main* Eversilver
    ...
    let builder = builder
        ...
        // Eversilver's child-webview/core teardown can run.
        .menu(macos_app_menu)
    ...
    let builder = builder
        ...
        // the vendored runtime asserts (Sentry EVERSILVER-TAURI-A — 442 events
    ...
    builder
        .setup(move |app| {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // containers (EVERSILVER-TAURI-AS: WSL2 user in BR) — the
            ...
            // core reads EVERSILVER_WEBVIEW_APIS_PORT on first connect, and
            ...
            std::env::set_var("EVERSILVER_CORE_RPC_URL", core_handle.rpc_url());
            ...
            // runtime with `EVERSILVER_CEF_PREWARM=0` if it regresses.
            {
                let prewarm_enabled = std::env::var("EVERSILVER_CEF_PREWARM")
                    .map(|v| {
                ...
                if prewarm_enabled {
                    ...
                    log::info!("[cef-prewarm] disabled via EVERSILVER_CEF_PREWARM");
                }
            }

            // Dev convenience: if EVERSILVER_DEV_AUTO_WHATSAPP=<account-id>
            ...
            if let Ok(account_id) = std::env::var("EVERSILVER_DEV_AUTO_WHATSAPP") {
            ...
            // Same dev helper, Slack flavour. EVERSILVER_DEV_AUTO_SLACK=<uuid>
            // opens the Slack account webview on startup so the CDP scanner
            // can iterate without manual UI clicks.
            if let Ok(account_id) = std::env::var("EVERSILVER_DEV_AUTO_SLACK") {
                let account_id = account_id.trim().to_string();
                if !account_id.is_empty() {
                    let app_handle = app.handle().clone();
                    tauri::async_runtime::spawn(async move {
            ...
            // Same dev helper, Telegram flavour. EVERSILVER_DEV_AUTO_TELEGRAM=<uuid>
            // opens the Telegram Web K account webview on startup so the CDP
            // scanner can iterate without manual UI clicks.
            if let Ok(account_id) = std::env::var("EVERSILVER_DEV_AUTO_TELEGRAM") {
                let account_id = account_id.trim().to_string();
                if !account_id.is_empty() {
                    let app_handle = app.handle().clone();
                    tauri::async_runtime::spawn(async move {
            ...
            // EVERSILVER_DEV_AUTO_GOOGLE_MEET=<uuid> opens the gmeet account
            ...
            if let Ok(account_id) = std::env::var("EVERSILVER_DEV_AUTO_GOOGLE_MEET") {
                let account_id = account_id.trim().to_string();
                if !account_id.is_empty() {
                    let app_handle = app.handle().clone();
                    tauri::async_runtime::spawn(async move {
            ...
            // Dev helper: EVERSILVER_DEV_AUTO_MEET_CALL=<https://meet.google.com/...>
                ...
                if !meet_url.is_empty() {
                    ...
                    tauri::async_runtime::spawn(async move {
                        ...
                        let args = meet_call::OpenWindowArgs {
                            ...
                            display_name: "Eversilver Dev".to_string(),
                        };
        ...
        .run(move |app_handle, event| match event {
            ...
            // (EVERSILVER-TAURI-2X — 21 events, Windows only).
            //
...
pub fn run_core_from_args(args: &[String]) -> Result<(), String> {
    // Core lives in-process: dispatch directly through the linked `eversilver_core`
    // library instead of shelling out to a separate binary. The Tauri main()
    // routes `Eversilver core <args>` here so users can still drive the core CLI
    // from the bundled app.
    eversilver_core::run_core_from_args(args).map_err(|e| format!("{e:#}"))
}
...
/// Resolve the Sentry environment tag from `EVERSILVER_APP_ENV` (runtime) or
/// `VITE_EVERSILVER_APP_ENV` (compile-time fallback). Defaults to
...
fn resolve_sentry_environment() -> String {
    ...
    if let Some(value) = option_env!("VITE_EVERSILVER_APP_ENV") {
        let trimmed = value.trim();
        if !trimmed.is_empty() {
            return trimmed.to_string();
        }
...
mod tests {
    ...
    fn core_rpc_url_returns_expected_format() {
        ...
        let original = std::env::var("EVERSILVER_CORE_RPC_URL").ok();

        std::env::set_var("EVERSILVER_CORE_RPC_URL", "http://localhost:9999/rpc");
        ...
        match original {
            Some(v) => std::env::set_var("EVERSILVER_CORE_RPC_URL", v),
            None => std::env::remove_var("EVERSILVER_CORE_RPC_URL"),
        }
    }
    ...
    fn overlay_parent_rpc_url_handles_empty() {
        ...
        std::env::set_var("EVERSILVER_CORE_RPC_URL", "");
        assert!(overlay_parent_rpc_url().is_none());

        std::env::set_var("EVERSILVER_CORE_RPC_URL", "   ");
        assert!(overlay_parent_rpc_url().is_none());

        std::env::set_var("EVERSILVER_CORE_RPC_URL", "http://127.0.0.1:7788/rpc");
        assert_eq!(
            overlay_parent_rpc_url(),
    ...
    #[test]
    fn sentry_environment_reads_eversilver_app_env() {
        let _g = ENV_LOCK.lock().unwrap();
        let key = "EVERSILVER_APP_ENV";
    ...
    #[test]
    fn sentry_environment_trims_whitespace_from_eversilver_app_env() {
        let _g = ENV_LOCK.lock().unwrap();
        let key = "EVERSILVER_APP_ENV";
        let original = std::env::var(key).ok();
    ...
    #[test]
    fn sentry_environment_skips_empty_eversilver_app_env() {
        let _g = ENV_LOCK.lock().unwrap();
        let key = "EVERSILVER_APP_ENV";
        let original = std::env::var(key).ok();
        std::env::set_var(key, "");
    ...
    #[test]
    fn sentry_environment_skips_whitespace_only_eversilver_app_env() {
        let _g = ENV_LOCK.lock().unwrap();
        let key = "EVERSILVER_APP_ENV";
        let original = std::env::var(key).ok();
        std::env::set_var(key, "   ");
    ...
    fn sentry_environment_defaults_to_production_when_unset() {
        let _g = ENV_LOCK.lock().unwrap();
        if option_env!("VITE_EVERSILVER_APP_ENV").is_some() {
            // A compile-time override is baked in; skip — the fallback path is
            // exercised by sentry_environment_skips_empty_eversilver_app_env.
            return;
        }
        let key = "EVERSILVER_APP_ENV";
        let original = std::env::var(key).ok();
    ...
    //    builds (issue EVERSILVER-TAURI-V). Tests target the pure
    ...
    // -------------------------------------------------------------------------
    // path_has_executable / deep-link xdg-mime pre-flight (EVERSILVER-TAURI-AS)
    // -------------------------------------------------------------------------
    ...
    /// case EVERSILVER-TAURI-AS protects against).
    #[cfg(target_os = "linux")]
    #[test]
