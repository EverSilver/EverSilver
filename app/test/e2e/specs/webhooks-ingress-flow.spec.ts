...
describe('Webhooks ingress surface (stub-level)', () => {
  ...
  it('exposes the stub webhook RPC surface with stable result and log shapes', async () => {
    ...
    const registrations = await callEversilverRpc('eversilver.webhooks_list_registrations', {});
    ...
    const logs = await callEversilverRpc('eversilver.webhooks_list_logs', { limit: 5 });
    ...
    const clear = await callEversilverRpc('eversilver.webhooks_clear_logs', {});
    expect(clear.ok).toBe(true);
    expect(clear.result?.result?.cleared).toBe(0);
    expect(clear.result?.logs?.[0]).toContain('webhooks.clear_logs removed 0');

    const unregister = await callEversilverRpc('eversilver.webhooks_unregister_echo', {
      tunnel_uuid: tunnelUuid,
    });
