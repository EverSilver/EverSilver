...
- **ServiceBlockingGate CORS errors** — The gate calls `eversilverServiceStatus()` and `eversilverAgentServerStatus()` at startup. These used `callCoreRpc()` which falls back to raw `fetch()` when socket isn't connected yet, causing CORS errors. Fix: route through `invoke('core_rpc_relay')` instead (Tauri IPC, no CORS).
...
- **`eversilver.agent_server_status` doesn't exist** — This RPC method is not registered in the core. The gate checks it but it always errors. The gate passes if either service is Running OR agent server is running OR core is reachable.
...
- **Always run checks before commit** — `pnpm workspace eversilver-app compile`, `pnpm lint`, `pnpm format:check`, `pnpm build`, `pnpm tauri dev`. Husky hooks enforce some but run all manually first.
...
- **PRs target upstream** — `eversilver/eversilver` main branch, not fork

## Local AI Presets

- **Tier system lives in `src/eversilver/local_ai/presets.rs`** — single source of truth for tier→model ID mapping. To change default models for a release, edit `all_presets()` there.
- **Device detection** uses `sysinfo` crate (`src/eversilver/local_ai/device.rs`). Apple Silicon = GPU always; others = best-effort.
- **`EVERSILVER_LOCAL_AI_TIER` env var** overrides the selected tier at config load time (in `load.rs`).
...
- **`pnpm core:stage` is a no-op echo** — there is no `target/debug/eversilver-core` binary to copy into `app/src-tauri/binaries/`. Rebuilding via `pnpm dev:app` is enough to pick up new RPC methods in the in-process server.
- **Token auth**: per-launch hex bearer in `EVERSILVER_CORE_TOKEN`, exposed to the renderer via `core_rpc_token` Tauri command. Always call RPC through `invoke('core_rpc_relay', { request: { method, params } })` — avoids the CORS preflight `fetch()` would trigger.
- **Stale-listener policy** (#1130): if the port is already in use, the handle probes `GET /` to decide if it's an Eversilver core, then term/force-kill with PID revalidation guarding against PID reuse. Set `EVERSILVER_CORE_REUSE_EXISTING=1` to attach to a manually-started `eversilver-core serve` for debugging.
- **Default port** `7788`. Stage token (when running standalone): `~/.eversilver-staging/core.token` under `EVERSILVER_APP_ENV=staging`.
...
- **`PersistGate loading={null}` causes flash** — Changed to `loading={<RouteLoadingScreen />}` (issue #413). `RouteLoadingScreen` accepts an optional `label` prop (defaults to "Initializing Eversilver...") and can be rendered with no props.
...
- **Full dark-to-light redesign shipped** — All pages, components, and settings panels converted from dark glass-morphism to clean light theme based on Figma designs by Mithil (`Eversilver-Prod` file, node `2094-250136` for tokens).
...
- **Banner dismiss state uses localStorage** (prefix `eversilver:upsell:`), not Redux — consistent with CLAUDE.md exception for ephemeral UI state.
...
- **`getTeamUsage()` now normalizes via `normalizeTeamUsage()`** — Added in issue #482. The Rust sidecar passes backend JSON through opaquely (`src/eversilver/team/ops.rs`), so the TS client must normalize field names and types. Pattern matches existing `normalizeCreditBalance()` in the same file. Any new billing API that returns raw backend data should follow the same normalize-at-the-client pattern.
...
- **`ProviderUserProfile.profile_url`** — New optional field on the struct in `src/eversilver/composio/providers/types.rs`. Providers should populate it when available from upstream profile payloads.
- **`identity_set` callback in default flow** — `ComposioProvider::on_connection_created()` in `src/eversilver/composio/providers/traits.rs` now calls `identity_set(&profile)` after profile fetch. `composio_get_user_profile` in `src/eversilver/composio/ops.rs` also routes persistence through `identity_set`.
...
- **Connected identities loader/renderer** — `src/eversilver/composio/providers/profile.rs` contains `load_connected_identities()` (reads `skill:*` facets) and `render_connected_identities_section()` (formats markdown for prompt injection). Keep rendering logic there, not in prompt modules.
...
- **Rust-side HTTP timeout is separate** — `src/eversilver/providers/compatible.rs` sets a 120s `reqwest` client timeout on LLM calls. Not changed in #715; relevant if a single LLM round-trip itself stalls for >2 min.
- **Manual cancel path** — `chatCancel()` in `app/src/services/chatService.ts` → `eversilver.channel_web_cancel` RPC → `cancel_chat()` in `src/eversilver/channels/providers/web.rs`. Fully implemented; the silence timer is an automatic fallback.

## Webhook & Cron Triggers (Issue #726)

- **Webhook bus was hardcoded 410** — `src/eversilver/webhooks/bus.rs` `WebhookRequestSubscriber::handle()` returned 410 "skill runtime removed" for ALL incoming webhooks. Now routes to echo/agent/skill/404 based on `TunnelRegistration.target_kind`.
...
- **`pnpm typecheck` script was renamed** — Check `app/package.json` for the current name; as of issue #830 work, use `pnpm workspace eversilver-app compile` for tsc checks.
...
- **Capability catalog must stay in sync** — `src/eversilver/about_app/catalog.rs` needs an entry when a new user-visible feature ships. GA was added there as part of issue #1479.
...
- **Config corruption recovery** — `parse_config_with_recovery` in `src/eversilver/config/schema/load.rs`: try primary → try `.bak` → archive corrupt file → `Config::default()`. Guarantees the app always starts even with a corrupt config.
...
- **Config test module path** — `eversilver::config::schema::load::tests`. Run with `cargo test -- config::schema::load::tests`.
...
- **Skills runtime removed** — the QuickJS / `rquickjs` runtime is gone; `src/eversilver/skills/` is metadata-only ("Legacy skill metadata helpers retained after QuickJS runtime removal"). Skill execution surfaces are being rebuilt; don't assume a `.skill` can run end-to-end without checking the current code.
